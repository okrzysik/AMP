...
ManagedEpetraMatrix::ManagedEpetraMatrix ( const ManagedEpetraMatrix &rhs ) :
    Matrix(),
    EpetraMatrix( rhs.d_pParameters->getEpetraRowMap(), rhs.d_pParameters->getEpetraColMap(), rhs.d_pParameters->entryList() ),
...
{
    AMP::shared_ptr<ManagedEpetraMatrixParameters> memp = AMP::dynamic_pointer_cast<ManagedEpetraMatrixParameters>( d_pParameters );
...
void ManagedEpetraMatrix::mult ( Vector::const_shared_ptr in, Vector::shared_ptr out )
{
    ...
    AMP::shared_ptr<const EpetraVector>  in_view = AMP::dynamic_pointer_cast<const EpetraVector>( EpetraVector::constView( in ) );
    AMP::shared_ptr<EpetraVector>  out_view = AMP::dynamic_pointer_cast<EpetraVector>( EpetraVector::view( out ) );
...
void  ManagedEpetraMatrix::getValuesByGlobalID ( int  num_rows, int num_cols, int *rows, int *cols, double *values ) const
{
    ...
    for (int i=0; i<num_rows; i++ ) {
        ...
        VerifyEpetraReturn( d_epetraMatrix->ExtractGlobalRowCopy( rows[i], numCols, numCols, &(row_values[0]), &(row_cols[0]) ), "getValuesByGlobalID" );
