...
/*! \page BuildOptions  AMP configure options
  ...
  packages are not included.  For example all packages require UTILS, and DISCRETIZATION requires MESH.  The packages are: <BR>
  ...
  \ref AMP::Discretization "DISCRETIZATION": The DISCRETIZATION package provides classes for managing DOFs 
  ...
  a given package.  All external packages can be included or disabled using the "-D USE_EXT_PACKAGE="
  flags.  Unless otherwise noted, all external packages take a second cmake argument "-D PACKAGE_DIRECTORY="
  specifying the directory where the given package is installs.  The current list of external packages is: <BR>
  BOOST: Boost is currently required to build AMP. <BR>
  MPI: MPI provides parallel capabilities for AMP.  If used, there are a number of additional optional flags
     that are used to configure MPI: <BR>
        &nbsp;&nbsp;&nbsp; "-D USE_MPI_FOR_SERIAL_TESTS=" indicates that we want to use mpi to run the serial tests in ctest.  Default is false. <BR>
        &nbsp;&nbsp;&nbsp; "-D MPIEXEC_NUMPROC_FLAG:STRING=" the flag to specify the flag mpi uses to specify the number of processors.  Default is "-np"<BR>
  ...
  TRILINOS: Trilinos is used throughout AMP to provide additional functionality.  It is used but not 
     required within LinearAlgebra, Operators and Solvers.  Compiling without Trilinos will result in reduced functionality.  <BR>
  PETSC: Petsc is used throughout AMP to provide additional functionality.  It is used but not 
     required within LinearAlgebra, Operators and Solvers.  Compiling without Petsc will result in reduced functionality.  <BR>
  LIBMESH: LibMesh is required for the Operators package.  It is currently used within Mesh for additional functionality.  <BR>
  ...
  "-D COMPILE_MODE=":  Required string indicating if we are building in "debug" or "optimized mode".  <BR>
  ...
  "-D TEST_MAX_PROCS="  Optional flag indicating the maximum number of processors for the tests (default: 32) <BR>
  "-D USE_STATIC="  Optional flag indicating if we are only using static libraries (default: false) <BR>
  "-D USE_FORTRAN="  Optional flag indicating if we want to include the fortran compiler (default: true) <BR>
