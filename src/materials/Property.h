...
public:
	...
	Property(
		...
		d_nparams(nparams), d_n_arguments(nargs), d_arguments(nargs),
	...
	{
		AMP_INSIST(d_variableNumberParameters, "changing number of parameters for this property not allowed");
	...
	{
		return d_n_arguments;
	}

	/** get the defaults */
	std::vector<Number> get_defaults()
	{
		return d_defaults;
	}
	...
	void set_translator(const std::map<std::string, std::string> &xlator)
	{
		...
		for (std::map<std::string, std::string>::const_iterator iter =
				xlator.begin(); iter != xlator.end(); ++iter)
		{
