...
namespace AMP
{
  namespace Materials
  {
    ...
    /**
     * \class          Property
     * \brief          Provides material properties of scalar, vector or tensor type.
	...
	public:
           ...
           */
			Property( const std::string name = std::string("NotDefined"),
					  ...
					  const bool hasScalar=true,
					  const bool hasVector=false,
					  const bool hasTensor=false
					):
						d_name(name), d_source(source),
						d_params(std::valarray<Number>(params, nparams)), d_nparams(nparams),
						d_n_arguments(nargs), d_arguments(nargs),
						d_defaults(nargs), d_ranges(nargs, std::vector<Number>(2)),
						d_hasScalarEval(hasVector), d_hasVectorEval(hasVector), d_hasTensorEval(hasTensor)
			{
			...
			virtual ~Property(){}
                        ...
                        void evalvActual(RETURN_VTYPE& r, const std::map< std::string, boost::shared_ptr<INPUT_VTYPE> >& args);

                        /* Loops through input vectors, calling the child eval function, returning vector results */
                        template <class INPUT_VTYPE, class RETURN_VTYPE>
                        void evalvActualVector(std::vector<boost::shared_ptr<RETURN_VTYPE> >& r, const std::map< std::string, boost::shared_ptr<INPUT_VTYPE> >& args);

                        /* Loops through input vectors, calling the child eval function, returning tensor results */
                        template <class INPUT_VTYPE, class RETURN_VTYPE>
                        void evalvActualTensor(std::vector<std::vector<boost::shared_ptr<RETURN_VTYPE> > >& r, const std::map< std::string, boost::shared_ptr<INPUT_VTYPE> >& args);
                         ...
                         */
                        virtual Number eval( std::vector<Number>& args);
                         ...
                         */
                        virtual std::vector<Number> evalVector( std::vector<Number>& args);
                         ...
                         */
                        virtual std::vector<std::vector<Number> > evalTensor( std::vector<Number>& args);
                         ...
                         *  returned in r[i].
                         */
                        void virtual evalv(std::vector< boost::shared_ptr<std::vector<Number> > >& r,
                                        const std::map< std::string, boost::shared_ptr<std::vector<Number> > >& args);

                        /** Wrapper function that calls evalvActualTensor for each argument set
                         ...
                         *  returned in (*r[j])[i].
                         */
                        void virtual evalv(std::vector< std::vector< boost::shared_ptr<std::vector<Number> > > >& r,
                                        const std::map< std::string, boost::shared_ptr<std::vector<Number> > >& args);
                         ...
                         *  returned in (*r[k][j])[i].
                         */
                        void virtual evalv(boost::shared_ptr<AMP::LinearAlgebra::Vector>& r,
                         ...
                         *  returned in (*r[j])[i].
                         */
                        void virtual evalv(std::vector<boost::shared_ptr<AMP::LinearAlgebra::Vector> >& r,
                                        const std::map< std::string, boost::shared_ptr<AMP::LinearAlgebra::Vector> >& args);
                         ...
                         *  returned in (*r[k][j])[i].
                         */
                        void virtual evalv(std::vector< std::vector< boost::shared_ptr<AMP::LinearAlgebra::Vector> > >& r,
                                        const std::map< std::string, boost::shared_ptr<AMP::LinearAlgebra::Vector> >& args);
			...
			std::valarray<Number> get_parameters()
			{
				return d_params;
			}
			...
			{
				std::map<std::string, size_t>::iterator it = d_argToIndexMap.find(argname);
				//AMP_INSIST(it != d_argToIndexMap.end(), std::string("argument ")+argname+std::string("is invalid"));
			...
			void set_translator(const std::map<std::string, std::string> &xlator) {
				...
				for (std::map<std::string, std::string>::const_iterator iter=xlator.begin(); iter!=xlator.end(); ++iter) {
					AMP_ASSERT(std::find(d_arguments.begin(), d_arguments.end(), iter->first) != d_arguments.end());
				}
				d_translator = xlator;
			}
			...
			/// indicates if a scalar eval function is available
			bool hasScalarEval(){return d_hasVectorEval;}

			/// indicates if a vector eval function is available
			bool hasVectorEval(){return d_hasVectorEval;}

			/// indicates if a tensor eval function is available
			bool hasTensorEval(){return d_hasTensorEval;}
	...
	private:
			...
			bool 								d_hasScalarEval;        ///< true if has a vector eval function
			bool 								d_hasVectorEval;        ///< true if has a vector eval function
			bool								d_hasTensorEval;        ///< true if has a tensor eval function
