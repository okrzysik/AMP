...
Journal of Nuclear Materials 394 (2009) 182--189");
	...
	static const double DEparams[9]={0.99672, 1.179e-5, -2.429e-9,  1.219e-12, 
									 0.99734, 9.082e-6, -2.705e-10, 4.391e-13, 10960};
	static const double TEparams[8]={1.1833e-5, -5.013e-9,  3.756e-12, -6.125e-17,
									 9.828e-6, -6.390e-10,  1.33e-12, -1.757e-17};
	static const double HCparams[9]={52.174, 45.806, 87.951e-3, -7.3461e-2, -84.241e-6, 31.542e-9, -2.6334e-12, -713910, -295090};
...
//=================== Functions =====================================================
	...
	inline double HeatCapacityPressureProp::eval( std::vector<double>& args ){
		...
		double T2 = T*T;
		double heatcp = p[0] + p[1]*x + (p[2] + p[3]*x)*T + (1-x)*(p[4]*T2 + p[5]*T2*T + p[6]*T2*T2) + (p[7] + p[8]*x)/T2;
		return heatcp;
	}
	...
	inline double DxThermalConductivityProp::eval( std::vector<double>& args ){
		...
		double DxThCond = ((-0.35355339059327373*atan(1.4142135623730951*exp(T*p[3])*
							sqrt(x/(p[0] + T*p[1]))*p[2])*sqrt(x/(p[0] + T*p[1])))/
							(exp(T*p[3])*p[2]) + (0.5*x)/(p[0] + T*p[1] +
							2.0*exp(2*T*p[3])*x*p[2]*p[2]))/x*x;
	...
	inline double DTThermalConductivityProp::eval( std::vector<double>& args ){
		...
		double DTThCond = (-0.5*p[1])/((p[0] + T*p[1])*(p[0] + T*p[1] +
							2.0*exp(2*T*p[3])*x*p[2]*p[2])) +
							(1.*p[3])/(p[0] + T*p[1] + 2.0*exp(2*T*p[3])*x*p[2]*p[2]) +
							(atan(1.4142135623730951*exp(T*p[3])*sqrt(x/(p[0] + T*p[1]))*p[2])*
							(-0.7071067811865475*p[0]*p[3] + p[1]*(-0.35355339059327373 -
							0.7071067811865475*T*p[3])))/(exp(T*p[3])*sqrt(x/(p[0] + T*p[1]))*
							(p[0] + T*p[1])*(p[0] + T*p[1])*p[2]) + 3*T*T*p[4];
