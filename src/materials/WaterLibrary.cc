...
//=================== Functions =====================================================
	...
	inline double TemperatureProp::eval( std::vector<double>& args ){
		...
		if (P < P_crit and H <= Hf) InLiquidRegion = true;
		if (P >= P_crit and H <= H_crit) InCriticalRegion = true;
		InAcceptableRegion = InLiquidRegion or InCriticalRegion;
		if (not InAcceptableRegion) AMP_ERROR("Liquid water temperature called, but water not in liquid or critical state.");
		...
		if (InLiquidRegion) // liquid region
		{
			for (size_t i=0; i<2; i++)
				for (size_t j=0; j<4; j++)
					T = T + a[i][j]*pow(P,i)*pow(H,j);
		}
		else if (InCriticalRegion) // critical region
		{
			for (size_t i=0; i<5; i++)
				for (size_t j=0; j<5; j++)
					T = T + b[i][j]*pow(P,i)*pow(H,j);
		}
	...
	inline double SaturatedLiquidEnthalpyProp::eval( std::vector<double>& args ){
		...
		Hf = 0;
		if (P >= Pmin and P < 900) // liquid region
		{
			for (size_t i=0; i<9; i++)
				Hf = Hf + a[i]*pow(log(P),i);
		}
		else if (P >= 900 and P < 2450)
		{
			for (size_t i=0; i<9; i++)
				Hf = Hf + b[i]*pow(log(P),i);
		}
		else if (P >= 2450 and P < P_crit)
		{
			for (size_t i=0; i<9; i++)
				Hf = Hf + c[i]*pow(pow((P_crit-P),0.41),i);
		}
		else
		{
	...
	inline double SpecificVolumeProp::eval( std::vector<double>& args ){
		...
		if (H <= 250 and H <= Hf) InLiquidRegion1 = true;
		if (H > 250 and H <= Hf and P < P_crit) InLiquidRegion2 = true;
		if (H > 250 and H <= H_crit and P > P_crit) InCriticalRegion = true;
		InAcceptableRegion = InLiquidRegion1 or InLiquidRegion2 or InCriticalRegion;
		if (not InAcceptableRegion) AMP_ERROR("Liquid water specific volume called, but water not in liquid or critical state.");
	...
	inline double ThermalConductivityProp::eval( std::vector<double>& args ){
		...
		for (size_t i=0; i<4; i++)
			Tratiosum = Tratiosum + a[i]*pow((T/Tstar),i);
	...
	inline double DynamicViscosityProp::eval( std::vector<double>& args ){
		...
		for (size_t k=0; k<4; k++)
			sum = sum + a[k]*pow(Tstar/T,k);
		double u0 = 1e-6*pow(T/Tstar,0.5)*pow(sum,-1.0);

		double expsum = 0;
		for (size_t i=0; i<5; i++)
			for (size_t j=0; j<6; j++)
				expsum = expsum + b[i][j]*pow(Tstar/T-1,j)*pow(rho/rhostar-1,i);
