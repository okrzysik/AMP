...
void checkConsistency( double h, double p, double T, bool &allCorrect, bool &allConsistent )
{
    ...
    tempMap.insert( std::make_pair(
        "enthalpy", AMP::shared_ptr<std::vector<double>>( new std::vector<double>( 1, h ) ) ) );
    tempMap.insert( std::make_pair(
    ...
    hMap.insert( std::make_pair(
        "temperature",
        AMP::shared_ptr<std::vector<double>>( new std::vector<double>( 1, tempOutput[0] ) ) ) );
    hMap.insert( std::make_pair(
        "pressure", AMP::shared_ptr<std::vector<double>>( new std::vector<double>( 1, p ) ) ) );
    ...
    try {
        ...
        AMP::shared_ptr<AMP::Materials::Material> mat =
            AMP::voodoo::Factory<AMP::Materials::Material>::instance().create(
                "WaterLibrary" );                                         // get water library
        PropertyPtr temperatureProperty = mat->property( "Temperature" ); // temperature property
        PropertyPtr liquidEnthalpyProperty =
        ...
        // Block for temporary variables
        {
            ...
            temperatureIdenticalArgMap.insert(
                std::make_pair( "enthalpy", enthalpyIdenticalInput ) );
            temperatureIdenticalArgMap.insert(
                std::make_pair( "pressure", pressureIdenticalInput ) );
            ...
            double viscosityKnown[n]    = { 0.000218794,
                                         7.72970e-5,
                                         0.000853838 };            // dynamic viscosity
            ...
            for ( size_t i = 0; i < n; i++ ) {
                ...
                // dynamic viscosity
                if ( !AMP::Utilities::approx_equal(
                         viscosityOutput[i], viscosityKnown[i], 0.01 ) ) {
                    ut.failure( "The answer is wrong." );
        ...
        // saturated liquid enthalpy, no argument
        {
            ...
            if ( !AMP::Utilities::approx_equal(
                     liquidEnthalpyOutput_def[0], knownSolution, 0.01 ) ) {
