...
int main( int argc, char **argv )
{
    ...
    auto mat  = AMP::voodoo::Factory<AMP::Materials::Material>::instance().create( "FixedFuel" );
    auto prop = mat->property( "ThermalConductivity" );

    // test property accessors
    std::string tcname = prop->get_name();
    std::string tcsorc = prop->get_source();
    good               = good && tcname == "FixedFuel_ThermalConductivity";
    ...
    auto tensorProperty = std::dynamic_pointer_cast<AMP::Materials::TensorProperty<double>>(
    ...
    if ( good )
        ut.passes( "basic tests of FixedFuel" );
    else
        ut.failure( "basic tests of FixedFuel" );
    ...
    if ( good )
        ut.passes( "basic tests of parameterized FixedFuel" );
    else
        ut.failure( "basic tests of parameterized FixedFuel" );
