...
int main ( int argc , char **argv )
{
   ...
   try
   {
       ...
       Material::shared_ptr mat = AMP::voodoo::Factory<AMP::Materials::Material>::instance().create("FixedFuel");
	   PropertyPtr prop = mat->property("ThermalConductivity");

	   // test property accessors
	   string tcname = prop->get_name();
	   string tcsorc = prop->get_source();
	   good = good and tcname == string("FixedFuel")+string("_ThermalConductivity");
	   ...
	   if (good) ut.passes("basic tests of FixedFuel");
	   else ut.failure("basic tests of FixedFuel");
	   ...
	   if (good) ut.passes("basic tests of parameterized FixedFuel");
	   else ut.failure("basic tests of parameterized FixedFuel");
   }
