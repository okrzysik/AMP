...
int main ( int argc , char **argv )
{
   ...
   try
   {
       ...
       Material::shared_ptr mat = AMP::voodoo::Factory<AMP::Materials::Material>::instance().create("Independent");
	   PropertyPtr prop = mat->property("ThermalConductivity");

	   // test property accessors
	   string tcname = prop->get_name();
	   string tcsorc = prop->get_source();
	   good = good and tcname == string("Independent")+string("_ThermalConductivity");
	   ...
	   std::vector<double> &uv=*new std::vector<double>(n);  // concntration
	   ...
	   for (size_t i=0; i<n; i++) {tv[i]=563.4;uv[i]=.05,bv[i]=0.;}  // set input arguments
	   ...
	   if (good) ut.passes("basic tests of Independent");
	   else ut.failure("basic tests of Independent");
	   ...
	   if (good) ut.passes("basic tests of parameterized Independent");
	   else ut.failure("basic tests of parameterized Independent");
   }
