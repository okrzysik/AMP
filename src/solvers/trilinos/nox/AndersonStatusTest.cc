...
NOX::StatusTest::StatusType AndersonStatusTest::checkStatus( const NOX::Solver::Generic &solver, NOX::StatusTest::CheckType checkType)
{
    ...
    Teuchos::RCP<const NOX::Abstract::Vector> prevSolVec = solver.getPreviousSolutionGroup().getXPtr();
    AMP_ASSERT( curSolVec  != Teuchos::null );
    AMP_ASSERT( prevSolVec != Teuchos::null );

    // Cast vectors to Thyra::Vector base class
    Teuchos::RCP<const NOX::Thyra::Vector> curSolThyraVec  = Teuchos::rcp_dynamic_cast<const NOX::Thyra::Vector>(curSolVec);
    Teuchos::RCP<const NOX::Thyra::Vector> prevSolThyraVec = Teuchos::rcp_dynamic_cast<const NOX::Thyra::Vector>(prevSolVec);
    ...
    const AMP::LinearAlgebra::ThyraVectorWrapper* curSolWrappedVec =
        dynamic_cast<const AMP::LinearAlgebra::ThyraVectorWrapper*>(curSolThyraVec->getThyraRCPVector().getRawPtr());
    const AMP::LinearAlgebra::ThyraVectorWrapper* prevSolWrappedVec =
        dynamic_cast<const AMP::LinearAlgebra::ThyraVectorWrapper*>(prevSolThyraVec->getThyraRCPVector().getRawPtr());
    AMP_ASSERT( curSolWrappedVec );
    AMP_ASSERT( prevSolWrappedVec );

    // Finally get the AMP vectors from wrapper
    AMP_ASSERT( curSolWrappedVec->numVecs() == 1 ); AMP_ASSERT( prevSolWrappedVec->numVecs() == 1 );
    ...
    {
        AMP::LinearAlgebra::Variable::shared_ptr thisVar( new AMP::LinearAlgebra::Variable(d_variableNames[i]) );
        AMP::LinearAlgebra::Vector::const_shared_ptr thisCurVec  = curSolAmpVec->constSubsetVectorForVariable(thisVar);
        AMP::LinearAlgebra::Vector::const_shared_ptr thisPrevVec = prevSolAmpVec->constSubsetVectorForVariable(thisVar);
        if( thisCurVec )
        {
...
NOX::StatusTest::StatusType AndersonStatusTest::getStatus() const
{
    return d_status;
}
