                                      ...
                                      std::vector<std::vector<double>> &slave2MasterFactors )
{
    ...
    for ( size_t i = 0; i < slaveNodes.size(); i++ ) {
        ...
        AMP_ASSERT( bndSideId < el.numSides() );
        //    std::cout<<"Slave node: "<<(slaveNodes[i])<<
        //      " is mapped to Side: "<<bndSideId<<" on Master element:
        //      "<<(slave2MasterElem[i])<<std::endl;
    } // end for i
}
                                     ...
                                     std::vector<size_t> &slave2MasterElem )
{
    ...
    for ( ; bnd != end_bnd; ++bnd ) {
        ...
        for ( int j = 0; j < 3; j++ ) {
            if ( currXYZ[j] <= minXYZ[j] ) {
                std::cout << "j = " << j << " currXYZ = " << currXYZ[j] << " minXYZ = " << minXYZ[j]
                          << std::endl;
            }
            if ( currXYZ[j] >= maxXYZ[j] ) {
                std::cout << "j = " << j << " currXYZ = " << currXYZ[j] << " maxXYZ = " << maxXYZ[j]
                          << std::endl;
            }
            AMP_ASSERT( currXYZ[j] > minXYZ[j] );
            AMP_ASSERT( currXYZ[j] < maxXYZ[j] );
        } // end for j
        ...
        if ( !( selectedElems.empty() ) ) {
            ...
            if ( selectedElems.size() > 1 ) {
                ...
                for ( size_t i = 0; i < selectedElems.size(); i++ ) {
                    AMP::Mesh::LibMeshElement el =
                        masterMeshAdapter->getElement( selectedElems[i] );
    ...
    AMP_ASSERT( slaveNodes.size() == slave2MasterElem.size() );
                               ...
                               double *rgH )
{
    ...
    for ( ; el != end_el; ++el ) {
        ...
        for ( int j = 0; j < 3; j++ ) {
            AMP_ASSERT( currMinXYZ[j] > minXYZ[j] );
            AMP_ASSERT( currMaxXYZ[j] < maxXYZ[j] );
        } // end for j
        unsigned int xyzs[3];
        unsigned int xyze[3];
        for ( int j = 0; j < 3; j++ ) {
