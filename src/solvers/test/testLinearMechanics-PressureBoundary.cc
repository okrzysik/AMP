...
void linearElasticTest(AMP::UnitTest *ut, std::string exeName, 
  ...
  if(exeName == "testLinearMechanics-PressureBoundary-Cylinder") {
    meshAdapter = manager->getMesh ( "cylinder" );
  } else {
    if(exeName == "testLinearMechanics-PressureBoundary-HaldenPellet") {
  ...
  //This has an in-place apply. So, it has an empty input variable and
  //the output variable is the same as what it is operating on. 
  pressureLoadVecOp->setVariable(bvpOperator->getOutputVariable());
  ...
  AMP::LinearAlgebra::Vector::shared_ptr mechPressureVec = meshAdapter->createVector( bvpOperator->getOutputVariable() );
...
/*  FILE* out1;
  out1 = fopen("LinearOperatorSolutionVector.txt","w");
  for(int i = 0; i < 4485; i++) {
    fprintf(out1,"%le\n",mechSolVec->getValueByLocalID(i));
  }
  fclose(out1);*/
