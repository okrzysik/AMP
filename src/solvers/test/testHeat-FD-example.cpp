...
class BESolver : public AMP::Solver::SolverStrategy {

public:
    BESolver( std::shared_ptr<AMP::Solver::SolverStrategyParameters> params_ ) : SolverStrategy( params_ ) { 

        AMP::pout << "BESolver:: empty constructor..." << std::endl;

    };

    // Used by OperatorFactory to create a BESolver
    static std::unique_ptr<AMP::Solver::SolverStrategy> create( std::shared_ptr<AMP::Solver::SolverStrategyParameters> params_) {  
        return std::make_unique<BESolver>( params_ ); };

    std::string type( ) const override { return "BESolver"; };

    void apply( std::shared_ptr<const AMP::LinearAlgebra::Vector> in, std::shared_ptr<AMP::LinearAlgebra::Vector> out ) override {

        AMP::pout << "BESolver::apply is empty with input..." << std::endl;

        auto DOF = in->getDOFManager();
        for ( auto i = DOF->beginDOF(); i != DOF->endDOF(); i++ ) 
            AMP::pout << in->getValueByGlobalID( i ) << " " << std::endl;
        AMP::pout << std::endl;

        // now this RHS vector gets passed to the underlying operator
        auto timeOperator = std::dynamic_pointer_cast<AMP::TimeIntegrator::TimeOperator>( d_pOperator );
        AMP_INSIST( timeOperator, "TimeOperator is NULL" );
        // auto in_true = timeOperator->d_pIntegratorSourceTerm();
        // AMP::pout << in_true << std::endl;
...
void driver(AMP::AMP_MPI comm, 
    ...
    ****************************************************************/
    /*
    In TimeIntegratorParameters we put a "solver" database and then this is registered through the solverFactory by BDF.

    We also give TimeIntegratorParameters an operator A, which appears in the ODEs as u'(t) = A(u), and then BDF will wrap this as a TimeOperator F, given by F(u;dt) = u - gamma*dt*A(u) - f, where f contains source terms generated from the integrator. The new solution at each time step is the solution u^{n+1} of F(u^{n+1}) = 0, which is equation our "solver" should solve

    1. Is the only way to solve this system Newton, even in the linear case? Suppose that A is a linear operator, then F is affine, NOT linear. Any standard linear preconditioner/solver requires that we have a linear system of the form C*x = b, but I don't know how to extract the RHS vector b == f in this case without doing extra work (Note that the integratorSourceTerm f is protected inside the TimeOperator class)

    2. How can we incorporate boundary conditions? The boundary conditions do not have a time derivative in them, so they're more of an algebraic constraint in a sense. What they look like is G(u) = c on the boundary, where u is the global vector, including interior and boundary points.
        So really, the boundary rows of the nonlinear system should not be of the form u - gamma*dt*A(u) - f = 0, but I cannot see how or where exactly I'm meant to change them to G(u) - c = 0.

    */


    // auto & solverFactory = AMP::Solver::SolverFactory::getFactory();
    // solverFactory.registerFactory( "BESolver", BESolver::create );

    int maxIntegratorSteps = 3;
    double finalTime = 0.5;
    //auto name = "BDF1";
    auto name = "ExplicitEuler";
    ...
    while ( T < finalTime ) {
        ...
        if ( timeIntegrator->checkNewSolution() ) {
            ...
            // TODO: I seem to need this below, but didn't previously...
