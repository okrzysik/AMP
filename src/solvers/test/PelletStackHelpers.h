...
void helperApplyBoundaryCorrections(std::vector<unsigned int> localPelletIds, 
...
void helperSetFrozenVectorForMaps(AMP::Mesh::MeshManager::shared_ptr manager, 
    AMP::AMP_MPI globalComm, std::vector<unsigned int> localPelletIds, 
...
void helperBuildPointLoadRHS(std::vector<unsigned int> localPelletIds, 
  ...
  for(size_t id = 0; id < localPelletIds.size(); id++) {
    ...
    loadOp->setVariable((nonlinearColumnOperator->getOperator(id))->getOutputVariable());
