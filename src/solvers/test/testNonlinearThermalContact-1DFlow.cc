...
//--------------------------------------
  ...
  AMP::shared_ptr<AMP::Operator::FlowFrapconOperator> flowOperator = AMP::dynamic_pointer_cast<AMP::Operator::FlowFrapconOperator>(AMP::Operator::OperatorBuilder::createOperator(meshAdapter2,
																						      "FlowFrapconOperator",
																						      input_db,
																						      flowtransportModel));
...
//-------------------------------------
  ...
  AMP::LinearAlgebra::Vector::shared_ptr robinRHSVec = AMP::LinearAlgebra::createVector( nodalDofMap2, thermalVolumeOperator2->getInputVariable() );
...
/*
          ...
          for( int i=1; i<flowVecSize ; i++) {
            Tc = resVec->getValueByLocalID(i) + (resVec->getValueByLocalID(i)- resVec->getValueByLocalID(i-1))*((Cp*G*De)/(4.*hclad*dz));
...
*/ 
	  ...
	  nonlinearSolver1->solve(RightHandSideVec1, TemperatureInKelvinVec1);
          ...
          std::cout<<"Norm of TemperatureInKelvinVec1: "<< TemperatureInKelvinVec1->L2Norm() << std::endl;
...
//          }
  ...
  if(testPassed)
  {
    ut->passes("Seggregated solve of Composite Operator using control loop of Nonlinear Thermal+Robin->Map->Gap->Map->Nonlinear Thermal+Robin .");
  }
  else
  {
    ut->failure("Seggregated solve of Composite Operator using control loop of Nonlinear Thermal+Robin->Map->Gap->Map->Nonlinear Thermal+Robin .");
  }
