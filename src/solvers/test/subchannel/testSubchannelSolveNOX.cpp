...
static void SubchannelSolve( AMP::UnitTest *ut, const std::string &exeName )
{
    ...
    { // Limit the scope so we can add an if else statement for Petsc vs NOX
        ...
        auto linearSolver =
        std::dynamic_pointer_cast<AMP::Solver::PetscSNESSolver>(nonlinearSolver)->getKrylovSolver();
        // set preconditioner
    ...
    if ( subchannelMesh.get() != nullptr ) {
        ...
        subchannelEnthalpy->setToScalar( AMP::Operator::Subchannel::scaleEnthalpy * hin, subchannelEnthalpy );
        subchannelPressure->setToScalar( AMP::Operator::Subchannel::scalePressure * Pout, subchannelPressure );
    ...
    // Solve
    PROFILE_START( "Solve" );
    AMP::pout << "Rhs norm: " << std::setprecision( 13 ) << globalRhsMultiVector->L2Norm(globalRhsMultiVector)
              << std::endl;
