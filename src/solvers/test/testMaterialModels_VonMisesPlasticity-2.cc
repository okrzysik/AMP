...
void myTest(AMP::UnitTest *ut, std::string exeName)
{
  ...
  double eph11[1000], sig11[1000], sig11_init[1000], slope[1000], sig11p[1000], eph11p[1000], slope_p[1000];
  ...
  for(int i = 0; i < (2 * max_load); i++) {
    ...
    if(-sig11_init[max_load + i] <= sigy1) {
      ...
      strain[0][0] = (plastic_strain_11) + (sig11_init[max_load + i] / E) + ((sig11_init[max_load + i] + sigy1) / H_a);
      strain[0][1] = (plastic_strain_22) + (-nu * (sig11_init[max_load + i] / E)) - ((1.0 / 2.0) * (sig11_init[max_load + i] + sigy1) / H_a);
      strain[0][2] = (plastic_strain_33) + (-nu * (sig11_init[max_load + i] / E)) - ((1.0 / 2.0) * (sig11_init[max_load + i] + sigy1) / H_a);
    }
    eph11[max_load + i] = strain[0][0];
    eph11p[max_load + i] = strain[0][0] - ((1.0/3.0) * (strain[0][0] + strain[0][1] + strain[0][2]));
    vmikModel->getInternalStress(strain, stress);

    sig11[max_load + i] = stress[0];
    sig11p[max_load + i] = stress[0] - ((1.0/3.0) * (stress[0] + stress[1] + stress[2]));
    vmikModel->globalReset();

    slope[max_load + i] = (sig11[max_load + i] - sig11[max_load + i - 1]) / (eph11[max_load + i] - eph11[max_load + i - 1]);
    slope_p[max_load + i] = (sig11p[max_load + i] - sig11p[max_load + i - 1]) / (eph11p[max_load + i] - eph11p[max_load + i - 1]);
  }
...
int main(int argc, char *argv[])
{
    ...
    exeNames.push_back("testMaterialModels_VonMisesPlasticity-2");
