...
void linearThermalTest(AMP::UnitTest *ut )
{
  ...
  // The analytical solution is:  T = T[r1] + (T[r2]-T[r1])*ln(r/r1)/a + b*[ (r*r-r1*r1) - c/a*ln(r/r1) ]
  //   c = r2*r2 - r1*r1
  //   b = power/(4 k)
  ...
  for( ; iterator != meshAdapter->endOwnedNode(); iterator++ ) {
    ...
    sol = T1InK + (T2InK-T1InK)/a * log(r/radius1InMeters) + b * ( (r*r-radius1InMeters*radius1InMeters) - c/a * log(r/radius1InMeters) );
