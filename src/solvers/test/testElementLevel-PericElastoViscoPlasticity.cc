...
void myTest(AMP::UnitTest *ut, std::string exeName) {
   ...
   AMP::shared_ptr<AMP::Mesh::initializeLibMesh> libmeshInit(new AMP::Mesh::initializeLibMesh(globalComm));
   {
      ...
      AMP::shared_ptr<AMP::Database> nonlinearMechanicsDatabase = input_db->getDatabase("NonlinearMechanicsOperator");
      std::string dirichletVectorCorrectionDatabaseName = nonlinearMechanicsDatabase->getString("BoundaryOperator");
      AMP::shared_ptr<AMP::Database> dirichletVectorCorrectionDatabase = input_db->getDatabase(dirichletVectorCorrectionDatabaseName);
      AMP::shared_ptr<AMP::Operator::NonlinearBVPOperator> nonlinearMechanicsBVPoperator = AMP::dynamic_pointer_cast<
      AMP::Operator::NonlinearBVPOperator>(AMP::Operator::OperatorBuilder::createOperator(meshAdapter,"NonlinearMechanicsOperator",input_db));
      ...
      AMP::shared_ptr<AMP::LinearAlgebra::MultiVariable>  multivariable = 
         AMP::dynamic_pointer_cast<AMP::LinearAlgebra::MultiVariable>(mechanicsNonlinearVolumeOperator->getInputVariable() );
    ...
    */
      ...
      for (int step=0;step<NumberOfLoadingSteps; step++)
      {
        ...
        dirichletVectorCorrectionDatabase->putDouble("value_6_0", (epsilon_dot * current_time));
...
int main(int argc, char *argv[])
{
    ...
    exeNames.push_back("testElementLevel-PericElastoViscoPlasticity-1");
