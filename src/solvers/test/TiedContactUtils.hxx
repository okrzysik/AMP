...
void createNodeSet(AMP::Mesh::MeshManager::Adapter::shared_ptr meshAdapter,
    const unsigned int bndId, std::vector<PointAndId> & nodeSet) {
  AMP::Mesh::MeshManager::Adapter::OwnedBoundaryNodeIterator bnd = meshAdapter->beginOwnedBoundary( bndId );
  AMP::Mesh::MeshManager::Adapter::OwnedBoundaryNodeIterator end_bnd = meshAdapter->endOwnedBoundary( bndId );
  ...
  std::cout<<"Created contact surface nodeset for "<<(meshAdapter->getMeshName())<<std::endl; 
}
...
void createMasterSlaveMap(AMP::Mesh::MeshManager::Adapter::shared_ptr masterMeshAdapter,
    ...
    const unsigned int masterId, const unsigned int slaveId,
    std::vector<unsigned int> & masterContactNodes, std::vector<unsigned int> & slaveContactNodes,
    std::vector<unsigned int> & masterVolumeNodes, std::vector<unsigned int> & slaveVolumeNodes) {
  ...
  std::cout<<"Master contact surface has " <<(masterContactSet.size())<<" nodes."<<std::endl;
  std::cout<<"Slave contact surface has " <<(slaveContactSet.size())<<" nodes."<<std::endl;

  const size_t numContactNodes = masterContactSet.size();

  AMP_INSIST( (masterContactSet.size() == slaveContactSet.size()), "Masterset size differs from Slaveset size" );
  ...
  for(size_t i = 0; i < numContactNodes; i++) {
    ...
    char msg[300];
    sprintf(msg, "Master point (%g, %g, %g) and Slave point (%g, %g, %g) differ.\n Master ID = %u and Slave ID = %u.\n",
        mX, mY, mZ, sX, sY, sZ, mId, sId);
