...
#include "solvers/CGSolverParameters.h"
...
public:
	...
	acceptable values (non-negative real values) 
     ...
     2. type: double, name : absolute_tolerance, default value of $1.0e-14$, absolute tolerance for CG solver
     acceptable values (non-negative real values)
     
     3. type: double, name : divergence_tolerance, default value of $1.0e3$, divergence tolerance for CG solver
     acceptable values (non-negative real values)
               	
     4. type: bool, name : uses_preconditioner, default value false
        acceptable values (false, true),
        side effect: if false sets string pc_type to "none"
		 
     5. type: string, name : pc_side, default value "RIGHT",
	 acceptable values ("RIGHT", "LEFT", "SYMMETRIC" )
         active only when uses_preconditioner set to true
     */
    CGSolver(AMP::shared_ptr<CGSolverParameters> parameters);
