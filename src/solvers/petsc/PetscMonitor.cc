...
void PetscMonitor::printKSPStatus( KSP ksp, int iteration, double L2norm )
{
    if ( d_comm.getRank()==0 ) {
        ...
        for (int i=0; i<((PetscObject)ksp)->tablevel; i++)
            indent += "  ";
        std::cout << indent << iteration << " KSP Residual norm " << std::scientific << std::setprecision(12) << L2norm << std::endl;
    }
}
void PetscMonitor::printSNESStatus( SNES snes, int iteration, double L2norm )
{
    if ( d_comm.getRank()==0 ) {
        ...
        for (int i=0; i<((PetscObject)snes)->tablevel; i++)
            indent += "  ";
        std::cout << indent << iteration << " SNES Function norm " << std::scientific << std::setprecision(12) << L2norm << std::endl;
    }
}
#endif
