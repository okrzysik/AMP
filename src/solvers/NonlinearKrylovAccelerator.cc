...
NonlinearKrylovAccelerator::NonlinearKrylovAccelerator(boost::shared_ptr<NonlinearKrylovAcceleratorParameters> params)
...
void
NonlinearKrylovAccelerator::getFromInput(const boost::shared_ptr<AMP::Database>& db)
{
  if (db->keyExists("max_vectors")) 
    ...
    {
      AMP_ERROR("NonlinearKrylovAccelerator" << " -- Key data `max_vectors'"
		 << " missing in input.");
  }
  ...
  if (db->keyExists("angle_tolerance")) 
    ...
    {
      AMP_ERROR("NonlinearKrylovAccelerator" << " -- Key data `angle_tolerance'"
		 << " missing in input.");
    }
...
void
NonlinearKrylovAccelerator::initialize(boost::shared_ptr<SolverStrategyParameters> parameters)
{

  boost::shared_ptr<NonlinearKrylovAcceleratorParameters> params = boost::dynamic_pointer_cast<NonlinearKrylovAcceleratorParameters>(parameters);
...
NonlinearKrylovAccelerator::correction(boost::shared_ptr<AMP::LinearAlgebra::Vector> &f)
{
  ...
  if (d_bContainsPendingVecs) 
    ...
    for (k = d_piNext[d_iFirstVectorIndex]; k != EOL; k = d_piNext[k])
      {
	d_ppdFunctionDifferenceInnerProducts[d_iFirstVectorIndex][k] = w->dot( *d_pFunctionDifferenceVectors[k] );
      }
				  ...
				  boost::shared_ptr<AMP::LinearAlgebra::Vector> u)
{
  ...
  // make the internal solution vector point to the output solution so we don't have to make a copy
  d_pvSolution = u;
  ...
  boost::shared_ptr<AMP::Operator::OperatorParameters> pc_parameters = d_pOperator->getJacobianParameters(d_pvSolution);
  boost::shared_ptr<AMP::Operator::Operator> pc_operator = d_pPreconditioner->getOperator();
  ...
  while( (d_iNonlinearIterationCount < d_iMaxIterations) && (residual_norm > d_dAbsoluteTolerance) )
    {
      if(!d_bFreezePc)
	{
      ...
      if(d_iDebugPrintInfoLevel>3)
	{
	  ...
	  std::cout << "NonlinearKrylovAccelerator::solve: L2 norm of correction from NLKAIN " << pcSolutionNorm << std::endl;
	}
      ...
      if(d_iDebugPrintInfoLevel>3)
	{
	  ...
	  std::cout << "NonlinearKrylovAccelerator::solve: L2 norm of corrected residual " << pcSolutionNorm << std::endl;
	}
      ...
      if (d_bPrintResiduals)
	{
	  AMP::pout << "Nonlinear Krylov iteration : " << d_iNonlinearIterationCount << ", residual: " << residual_norm << std::endl;
	}
...
void
NonlinearKrylovAccelerator::restart(void)
{
  int k;
...
*/
double 
NonlinearKrylovAccelerator::getAbsoluteTolerance() const
{
   return(d_dAbsoluteTolerance);
}

void 
NonlinearKrylovAccelerator::setAbsoluteTolerance(double abs_tol)
{
   d_dAbsoluteTolerance = abs_tol;
}

double 
NonlinearKrylovAccelerator::getRelativeTolerance() const
{
   return(d_dRelativeTolerance);
}

void 
NonlinearKrylovAccelerator::setRelativeTolerance(double rel_tol)
{
...
int 
NonlinearKrylovAccelerator::getMaxNonlinearIterations() const
{
   return(d_iMaxIterations);
}

void 
NonlinearKrylovAccelerator::setMaxNonlinearIterations(int max_nli)
{
   d_iMaxIterations = max_nli;
}

int 
NonlinearKrylovAccelerator::getMaxFunctionEvaluations() const
{
   return(d_iMaximumFunctionEvals);
}

void 
NonlinearKrylovAccelerator::setMaxFunctionEvaluations(int max_feval)
{
...
void 
NonlinearKrylovAccelerator::putToDatabase(boost::shared_ptr<AMP::Database>& db)
{

  AMP_INSIST(db.get()!=NULL,"database object cannot be NULL");

   db->putInteger("d_MaxIterations",
		  d_iMaxIterations);
