...
//--------------------------------------------------
  ...
  linearThermalOperator = AMP::dynamic_pointer_cast<AMP::Operator::LinearBVPOperator>(AMP::Operator::OperatorBuilder::createOperator(meshAdapter, "LinearThermalOperator" , input_db, thermalModel));
  linearOxygenOperator = AMP::dynamic_pointer_cast<AMP::Operator::LinearBVPOperator>(AMP::Operator::OperatorBuilder::createOperator(meshAdapter, "LinearOxygenOperator" , input_db, oxygenModel));
  //----------------------------------------------------------------------------------------------------------------------------------------------//
  ...
  massThermalOp = AMP::dynamic_pointer_cast<AMP::Operator::LinearBVPOperator>(AMP::Operator::OperatorBuilder::createOperator(meshAdapter, "MassThermalOperator", input_db,massThermalModel));
  massOxygenOp = AMP::dynamic_pointer_cast<AMP::Operator::LinearBVPOperator>(AMP::Operator::OperatorBuilder::createOperator(meshAdapter, "MassOxygenOperator" , input_db,massOxygenModel));
  ...
  AMP::shared_ptr<AMP::TimeIntegrator::ColumnTimeOperator> columnLinearTimeOperator( new AMP::TimeIntegrator::ColumnTimeOperator(timeOperatorParameters));
  
  // ---------------------------------------------------------------------------------------
  // create vectors for initial conditions (IC) and time derivative at IC
  AMP::shared_ptr<AMP::Operator::DiffusionNonlinearFEOperator> thermalVolumeOperator = AMP::dynamic_pointer_cast<AMP::Operator::DiffusionNonlinearFEOperator>(nonlinearThermalOperator->getVolumeOperator());
  AMP::shared_ptr<AMP::Operator::FickSoretNonlinearFEOperator> oxygenVolumeOperator = AMP::dynamic_pointer_cast<AMP::Operator::FickSoretNonlinearFEOperator>(nonlinearOxygenOperator->getVolumeOperator());
  ...
  AMP::shared_ptr<AMP::LinearAlgebra::MultiVariable> outputVar = AMP::dynamic_pointer_cast<AMP::LinearAlgebra::MultiVariable>(columnNonlinearRhsOperator->getOutputVariable());
  ...
  AMP::shared_ptr<AMP::Operator::VolumeIntegralOperator> sourceOperator = AMP::dynamic_pointer_cast<AMP::Operator::VolumeIntegralOperator>(AMP::Operator::OperatorBuilder::createOperator(meshAdapter, "VolumeIntegralOperator" , input_db, sourceTransportModel));
  ...
  AMP::shared_ptr<AMP::LinearAlgebra::Vector> thermalIC = initialCondition->select( vectorSelector1, (outputVar->getVariable(0))->getName() );
  AMP::shared_ptr<AMP::LinearAlgebra::Vector> oxygenIC = initialCondition->select( vectorSelector2, (outputVar->getVariable(1))->getName() );
  int counter=0;     
  ...
  while(pIDATimeIntegrator->getCurrentTime() < pIDATimeIntegrator->getFinalTime())
    ...
    ut->failure("Tested IDATimeIntegrator's advanceSolution. FAIL!!");
      ...
      AMP::shared_ptr<AMP::LinearAlgebra::Vector> currentThermalSolution = currentSolution->subsetVectorForVariable(thermalVolumeOperator->getOutputVariable());
      AMP::shared_ptr<AMP::LinearAlgebra::Vector> currentOxygenSolution = currentSolution->subsetVectorForVariable(oxygenVolumeOperator->getOutputVariable());
