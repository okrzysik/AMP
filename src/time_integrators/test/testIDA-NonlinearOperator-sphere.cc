...
#define __INIT_FN__(x,y,z) ( 1 - sqrt(x*x + y*y + z*z)*sqrt(x*x + y*y + z*z)*sqrt(x*x + y*y + z*z) )
#define __INIT_PRIME_FN__(x,y,z) ( sqrt(x*x + y*y + z*z)*sqrt(x*x + y*y + z*z)*sqrt(x*x + y*y + z*z) )

void IDATimeIntegratorTest(AMP::UnitTest *ut )
{
    std::string input_file = "input_testIDA-NonlinearOperator-sphere";
    std::string log_file = "output_testIDA-NonlinearOperator-sphere";
    ...
    AMP::shared_ptr<AMP::Operator::NonlinearBVPOperator> nonlinearOperator = AMP::dynamic_pointer_cast<
    ...
    AMP::shared_ptr<AMP::InputDatabase> linearOp_db =  AMP::dynamic_pointer_cast<AMP::InputDatabase>(input_db->getDatabase("LinearOperator"));
    linearOperator = AMP::dynamic_pointer_cast<AMP::Operator::LinearBVPOperator>(AMP::Operator::OperatorBuilder::createOperator(meshAdapter, linearOp_db, elementModel));
    ...
    massOperator = AMP::dynamic_pointer_cast<AMP::Operator::MassLinearFEOperator>(AMP::Operator::OperatorBuilder::createOperator(meshAdapter, massOp_db,massElementModel));
    ...
    AMP::shared_ptr<AMP::Database> sourceDatabase1 = input_db->getDatabase("VolumeIntegralOperator1");
    AMP::shared_ptr<AMP::Operator::VolumeIntegralOperator> sourceOperator1 = AMP::dynamic_pointer_cast<AMP::Operator::VolumeIntegralOperator>(AMP::Operator::OperatorBuilder::createOperator(meshAdapter, sourceDatabase1, manufacturedSourceModel1));
    ...
    AMP::shared_ptr<AMP::Database> sourceDatabase2 = input_db->getDatabase("VolumeIntegralOperator2");
    AMP::shared_ptr<AMP::Operator::VolumeIntegralOperator> sourceOperator2 = AMP::dynamic_pointer_cast<AMP::Operator::VolumeIntegralOperator>(AMP::Operator::OperatorBuilder::createOperator(meshAdapter, sourceDatabase2, manufacturedSourceModel2));
    ...
    AMP::shared_ptr<AMP::Operator::DiffusionNonlinearFEOperator> thermalVolumeOperator = AMP::dynamic_pointer_cast<AMP::Operator::DiffusionNonlinearFEOperator>(nonlinearOperator->getVolumeOperator());
    ...
    if (*inputVar==*inputVar_mass){
     ...
     AMP::LinearAlgebra::Variable::shared_ptr inputVarSource1 = sourceOperator1->getInputVariable(AMP::Operator::Diffusion::TEMPERATURE);
    ...
    AMP::LinearAlgebra::Vector::shared_ptr  scratchVec2 = meshAdapter->createVector(inputVar);
    AMP::LinearAlgebra::Vector::shared_ptr   f = meshAdapter->createVector(outputVar);
    
    cout << "*(initialCondition->getVariable()) == *inputVarSource1 = " << (*(initialCondition->getVariable()) == *inputVarSource1) << endl;    
    cout << "initialCondition->getVariable() = " << initialCondition->getVariable() << endl;
    cout << "sourceOperator1->getInputVariable(AMP::Operator::Diffusion::TEMPERATURE) = " << sourceOperator1->getInputVariable(AMP::Operator::Diffusion::TEMPERATURE) << endl;
    //----------------------------------------------------------------------------------------------------------------------------------------------//
  ...
  AMP::shared_ptr<AMP::ManagedEpetraMatrix> massMatrix = AMP::dynamic_pointer_cast<AMP::ManagedEpetraMatrix>(massOperator->getMatrix());
  cout << "massMatrix->L1Norm() = " << massMatrix->L1Norm() << endl;

     massOperator->apply(f,scratchVec,scratchVec2,1.0,0.0);
    cout << "scratchVec2->max() = " << scratchVec2->max() << endl;
     scratchVec->setToScalar(1.0);
    massOperator->apply(f,scratchVec,scratchVec2,1.0,0.0);
        cout << "scratchVec2->max() = " << scratchVec2->max() << endl;    
    cout << "initialCondition->max() = " << initialCondition->max() << endl;
    cout << "initialCondition->min() = " << initialCondition->min() << endl;    
    cout << "initialConditionPrime->max() = " << initialConditionPrime->max() << endl;
    ...
    AMP::shared_ptr<AMP::Operator::Operator> linearTimeOperator( new AMP::TimeIntegrator::LinearTimeOperator(timeOperatorParameters));
    // ---------------------------------------------------------------------------------------
    ...
    int j=1;
     ...
     cal = pIDATimeIntegrator->getCurrentSolution()->getValueByGlobalID( dof_map->getGlobalID( iterator->globalID(), 0 ) );
     ...
     scratchVec->setValueByGlobalID(dof_map->getGlobalID( iterator->globalID(), 0 ), fabs(cal - sol)/fabs(sol));
