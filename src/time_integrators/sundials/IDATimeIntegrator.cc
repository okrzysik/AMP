...
void IDATimeIntegrator::initializeIDA()
{
    ...
    ierr = IDAInit(d_ida_mem, IDAResTrial, d_initial_time, 
    ...
    if (d_bCallCalcIC) {
        ...
        AMP_ASSERT(ierr==IDA_SUCCESS);
        ierr = IDAGetConsistentIC(d_ida_mem, 
            pSundials_sol->castTo<AMP::LinearAlgebra::SundialsVector>().getNVector(), 
            pSundials_sol_prime->castTo<AMP::LinearAlgebra::SundialsVector>().getNVector() );
        AMP_ASSERT(ierr==IDA_SUCCESS);
    }
...
int IDATimeIntegrator::advanceSolution( const double dt, const bool /* first_step */ )
{
    ...
    retval = IDASolve(d_ida_mem, hcur, &d_current_time, 
        ptr_y->castTo<AMP::LinearAlgebra::SundialsVector>().getNVector(),
        ptr_ydot->castTo<AMP::LinearAlgebra::SundialsVector>().getNVector(), IDA_ONE_STEP );
    //should be fixed.
                                    ...
                                    N_Vector /* tmp1 */, N_Vector /* tmp2 */, N_Vector /* tmp3 */)
{
    ...
    if (OrderHasChanged || StepSizeHasChanged)
    {
        ...
        //              AMP::dynamic_pointer_cast<AMP::TimeIntegrator::LinearTimeOperator> ( pSolver->getOperator() );          
...
int IDATimeIntegrator::IDAPrecSolve( realtype , N_Vector , N_Vector , N_Vector ,
    N_Vector rvec, N_Vector zvec, realtype , realtype , void *user_data, N_Vector )
{
    AMP::LinearAlgebra::ExternalVectorDeleter d;
