...
void OxideModel::get_equilibrium_concetration( double T, double *Ci )
{
    ...
    if ( source == 1 ) {
        ...
        if ( T >= 1346 ) {
            Ci[3] = (-0.2263+sqrt(T/63.385-16.877))*0.0649; // Oxygen concentration in the alpha at the beta interface (g/cm^3)
        } else if ( T >= 1073 ) {
    ...
    if ( source == 1 ) {
        // "Zirconium Metal-Water Oxidation Kinetics IV. Raction Rate Studies", p. 36.
        if ( T >= 1646 ) {
            Ci[4] = ((T-1081.7)/491.159)*0.0649; // Oxygen concentration in the alpha at the beta interface (g/cm^3)
        } else if ( T >= 1506 ) {
            Ci[4] = (-0.00428+sqrt(T/392.46-3.1417))*0.0649; // Oxygen concentration in the alpha at the beta interface (g/cm^3)
        } else {
            AMP_ERROR("Invalid temperature");
        }
    } else if ( source == 2 ) {
...
************************************************************************/
int OxideModel::integrateOxide( double dT, int N, const int *N2, const double *x0, const double *Cb, 
    double **C0, const double *D, double **C1, double *x1, double *v1 )
{
    ...
    while ( t < dT ) {
        ...
        for (int i=0; i<N; i++) {
            ...
            double dt_Diff = computeDiffustionTimestep( N2[i], &x1[i+0], &Cb[2*i], C1[i], D[i] );
...
************************************************************************/
void OxideModel::computeVelocity( const int N, const double *x, const double *Cb, 
    const int *Nl, double const* const* C, const double *D, double *v )
{
    ...
    for (int i=0; i<N; i++) {
        double h = 0.5*(x[i+1]-x[i])/((double)Nl[i]);     // Spacing between endpoints and nearest zone (0.5*h0)
...
************************************************************************/
double OxideModel::computeDiffustionTimestep( const int N, const double x[2], 
...
************************************************************************/
void OxideModel::solveLinearDiffusionLayer( const int N, const double dt, const double x0[2], 
    ...
    double *C1, double *x1 )
{
    ...
    rhs[0] = C0[0] + 0.5*dt/(h0*h0)*(Db[1]*(C0[1]-C0[0])-Db[0]*2*(C0[0]-Cb[0])) + 0.5*dt/(h1*h1)*Db[0]*2*Cb[0];
    if ( vi >= 0 )
    ...
    for (int i=1; i<N-1; i++) {
        ...
        rhs[i] = C0[i] + 0.5*dt/(h0*h0)*(Db[i+1]*(C0[i+1]-C0[i])-Db[i]*(C0[i]-C0[i-1]));
        if ( vi >= 0 )
    ...
    rhs[N-1] = C0[N-1] + 0.5*dt/(h0*h0)*(Db[N]*2*(Cb[1]-C0[N-1])-Db[N]*(C0[N-1]-C0[N-2])) + 0.5*dt/(h1*h1)*Db[N]*2*Cb[1];
    if ( vi >= 0 )
        rhs[N-1] += 0.5*dt/h0*vi*2*(Cb[1]-C0[N-1]) + 0.5*dt/h1*vi*2*Cb[1];
    else
    ...
    vi = v[0] + (Nd-0.5)/Nd*(v[1]-v[0]);
    if ( vi >= 0 ) {
        diag[N-1] = 1.0 + 0.5*dt/(h1*h1)*(2.0*Db[N]+Db[N-1]) + 0.5*dt/h1*vi;
