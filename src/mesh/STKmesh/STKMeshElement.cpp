...
STKMeshElement::STKMeshElement( int dim,
                                stk::mesh::Entity *STKmesh_element,
                                unsigned int rank,
                                MeshID meshID,
                                const STKMesh *mesh )
    : d_dim( dim ),
...
STKMeshElement::STKMeshElement( int dim,
                                std::shared_ptr<stk::mesh::Entity> STKmesh_element,
                                unsigned int rank,
                                MeshID meshID,
                                const STKMesh *mesh )
    : d_dim( dim ),
...
std::vector<std::shared_ptr<MeshElement>> STKMeshElement::getNeighbors() const
{
    ...
    if ( d_globalID.type() == GeomType::Vertex ) {
        ...
        adjacent_entities.resize(
            std::unique( adjacent_entities.begin(), adjacent_entities.end() ) -
            adjacent_entities.begin() );
        adjacent_entities.erase(
            std::find( adjacent_entities.begin(), adjacent_entities.end(), elem ) );
...
double STKMeshElement::volume() const
{
    ...
    const CartesianField *coordinates =
        d_mesh->d_STKMeshMeta->get_field<CartesianField>( "coordinates" );
    ...
    Intrepid::FunctionSpaceTools::computeCellMeasure<double>(
        weightedMeasure, jacobian_det, cub_weights );
    Intrepid::FunctionSpaceTools::integrate<double>(
        volume, onesLeft, weightedMeasure, Intrepid::COMP_BLAS );
...
bool STKMeshElement::isOnBoundary( int ) const { return isOnSurface(); }
