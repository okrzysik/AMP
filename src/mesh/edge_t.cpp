                           ...
                           double tolerance )
{
    ...
    double distance_to_containing_line =
        compute_distance_to_containing_line( point_in_containing_plane );
    ...
    if ( distance_to_containing_line + tolerance > 0.0 ) {
        ...
        double position_on_containing_line =
            compute_scalar_product( &( tmp[0] ), &( direction[0] ) );
    ...
    for ( unsigned int i = 0; i < 3; ++i ) {
        projection[i] = point_in_containing_plane[i];
    }
    return -1;
}
