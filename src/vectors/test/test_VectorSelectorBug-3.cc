...
{
    std::string const exeName = "test_VectorSelectorBug-3";
    ...
    // Subset the mesh
    std::string const subsetMeshName = inputDatabase->getString("SubsetMeshName");
    AMP::Mesh::Mesh::shared_ptr subsetMesh = mesh->Subset(subsetMeshName);
    int const boundaryID = inputDatabase->getInteger("BoundaryID");
    AMP::Mesh::Mesh::shared_ptr boundaryMesh = 
        subsetMesh->Subset(mesh->getBoundaryIDIterator(AMP::Mesh::Volume, boundaryID));

    // Check mesh sizes
    AMP::pout<<"mesh volumes = "<<mesh->numGlobalElements(AMP::Mesh::Volume)<<"\n";
    AMP::pout<<"subset mesh volumes = "<<subsetMesh->numGlobalElements(AMP::Mesh::Volume)<<"\n";
    ...
    AMP::pout<<"subset mesh vertices = "<<subsetMesh->numGlobalElements(AMP::Mesh::Vertex)<<"\n";
