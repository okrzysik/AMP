...
template<typename TYPE>
bool VectorOperationsCuda<TYPE>::checkData( const VectorOperations& x, const VectorOperations& y ) const
{
    return x.getVectorData()->numberOfDataBlocks()==1 &&
           y.getVectorData()->numberOfDataBlocks()==1 &&
           d_VectorData->numberOfDataBlocks()==1;
}
...
double VectorOperationsCuda<TYPE>::localMin( void ) const
{
    ...
    if ( checkData( ) ) {
        ...
        result = thrust::reduce( thrust::device, data, data + N,
            std::numeric_limits<TYPE>::max(), thrust::minimum<TYPE>() );
    } else {
        // Default to VectorOperationsDefault (on cpu)
...
double VectorOperationsCuda<TYPE>::localMax( void ) const
{
    ...
    if ( checkData( ) ) {
        ...
        size_t N   = d_VectorData->sizeOfDataBlock( 0 );
        result = thrust::reduce( thrust::device, data, data + N,
            -std::numeric_limits<TYPE>::max(), thrust::maximum<TYPE>() );
    } else {
        // Default to VectorOperationsDefault (on cpu)
...
double VectorOperationsCuda<TYPE>::localL1Norm( void ) const
{
    ...
    if ( checkData( ) ) {
        ...
        result = thrust::transform_reduce( thrust::device, data, data + N,
            lambda, (TYPE) 0, thrust::plus<TYPE>() );
    } else {
        // Default to VectorOperationsDefault (on cpu)
...
double VectorOperationsCuda<TYPE>::localMaxNorm( void ) const
{
    ...
    if ( checkData( ) ) {
        ...
        auto lambda = [=] __device__ (TYPE x) { return x<0 ? -x:x; };
        result = thrust::transform_reduce( thrust::device, data, data + N,
            lambda, (TYPE) 0, thrust::maximum<TYPE>() );
    } else {
        // Default to VectorOperationsDefault (on cpu)
...
double VectorOperationsCuda<TYPE>::localMinQuotient( const VectorOperations &x ) const
{
    ...
    if ( checkData( ) ) {
        ...
        const TYPE *data2 = x.getVectorData()->getRawDataBlock<TYPE>( 0 );
        size_t N = d_VectorData->sizeOfDataBlock( 0 );
        result = thrust::inner_product( thrust::device, data1, data1 + N, data2,
            std::numeric_limits<TYPE>::max(), thrust::minimum<TYPE>(), thrust::divides<TYPE>() );
    } else {
        // Default to VectorOperationsDefault (on cpu)
...
double VectorOperationsCuda<TYPE>::localWrmsNorm( const VectorOperations &x ) const
{
    ...
    if ( checkData( ) ) {
        ...
        size_t N = d_VectorData->sizeOfDataBlock( 0 );
        result = thrust::inner_product( thrust::device, data1, data1 + N, data2,
            0, thrust::plus<TYPE>(), thrust_wrs<TYPE>() );
    } else {
        // Default to VectorOperationsDefault (on cpu)
...
void VectorOperationsCuda<TYPE>::multiply( const VectorOperations &x, const VectorOperations &y )
{
    if ( checkData(x,y) ) {
        ...
        const TYPE *ydata = y.getVectorData()->getRawDataBlock<TYPE>( 0 );
        size_t N = d_VectorData->sizeOfDataBlock( 0 );
        thrust::transform( thrust::device, xdata, xdata+N, ydata, data, thrust::multiplies<TYPE>() );
        cudaDeviceSynchronize();
    } else {
        // Default to VectorOperationsDefault (on cpu)
...
template<typename TYPE>
void VectorOperationsCuda<TYPE>::axpy( double alpha_in, const VectorOperations &x, const VectorOperations &y )
{
