    ...
    : NativeVector(), ThyraVector(), VectorEngine()
{
    ...
    d_CommList =
        AMP::shared_ptr<CommunicationList>( new CommunicationList( communicationListParams ) );
    d_DOFManager = AMP::shared_ptr<Discretization::DOFManager>(
        new Discretization::DOFManager( params->d_local, params->d_comm ) );
