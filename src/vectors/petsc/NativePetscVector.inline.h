...
inline size_t NativePetscVector::numberOfDataBlocks () const 
{ 
    return 1; 
}
...
inline AMP_MPI NativePetscVector::getComm () const
{
...
inline boost::shared_ptr<ParameterBase> NativePetscVector::getParameters ()
{
    return d_pParameters;
}


inline void NativePetscVector::aliasVector(Vector &)
{
...
inline void NativePetscVector::copyVector(const Vector &src_vec)
{
    resetArray();
...
inline void  NativePetscVector::setLocalValuesByGlobalID(int num, size_t *indices , const double *vals)
{
    INCREMENT_COUNT("Virtual");
    resetArray();
...
inline void  NativePetscVector::addLocalValuesByGlobalID(int num, size_t *indices , const double *vals)
{
    INCREMENT_COUNT("Virtual");
    resetArray();
    if ( sizeof(size_t) == sizeof(PetscInt) ) {
