               ...
               ,         public boost::enable_shared_from_this<Vector>
  {
    public:
      ...
      /** \brief Allocate space in the same fashion as <i>this</i>
        ...
        * \details  This will allocate new space with identical layout as <i>this</i>.
        * \return  A Vector shared pointer
        * It will have the same number of blocks, each with the same engines and same number of entries.  The vector will be associated with a clone of the same Variable with the given name
       */
       ...
       */
      double dot ( const Vector::shared_ptr &x );
