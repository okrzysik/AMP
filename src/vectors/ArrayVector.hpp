...
template<typename T, typename FUN, typename Allocator>
ArrayVector<T, FUN, Allocator>::ArrayVector(
    std::shared_ptr<ArrayVectorData<T, FUN, Allocator>> data )
    : Vector()
{
    setVectorData( data );
    d_VectorOps = std::make_shared<VectorOperationsDefault<T>>();
...
void ArrayVector<T, FUN, Allocator>::swapVectors( Vector &rhs )
{
    ...
    auto otherArray =
        std::dynamic_pointer_cast<ArrayVectorData<T, FUN, Allocator>>( rhs.getVectorData() )
            ->getArray();
    // reset views
