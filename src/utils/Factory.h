...
private:
    typedef typename std::map< Key, AMP::shared_ptr< RegistrationBase<BaseClass,Key,0> > > RegistrationMap;
...
struct Registration<BaseClass,Derived,Key,0> : public RegistrationBase<BaseClass,Key,0> {
    Registration(const Key& key) {
        try {
            Factory<BaseClass,Key,0> ::instance().Register( key, AMP::shared_ptr< RegistrationBase<BaseClass,Key,0> >(new Registration<BaseClass,Derived,Key,0>) ); \
        }
        catch (...){}
    }
  ...
  private:
    ...
    Registration<BaseClass,Derived,Key,0>& operator=( const Registration<BaseClass,Derived,Key,0>& );
};
