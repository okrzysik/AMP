...
template <typename T> static bool test_copy( int N, T& error );
template <typename T> static bool test_scal( int N, T& error );
template <typename T> static bool test_nrm2( int N, T& error );
template <typename T> static bool test_asum( int N, T& error );
template <typename T> static bool test_dot(  int N, T& error );
template <typename T> static bool test_axpy( int N, T& error );
template <typename T> static bool test_gemv( int N, T& error );
template <typename T> static bool test_gemm( int N, T& error );
template <typename T> static bool test_gesv( int N, T& error );
template <typename T> static bool test_gtsv( int N, T& error );
template <typename T> static bool test_gbsv( int N, T& error );
template <typename T> static bool test_getrf( int N, T& error );
template <typename T> static bool test_gttrf( int N, T& error );
template <typename T> static bool test_gbtrf( int N, T& error );
template <typename T> static bool test_getrs( int N, T& error );
template <typename T> static bool test_gttrs( int N, T& error );
template <typename T> static bool test_gbtrs( int N, T& error );
template <typename T> static bool test_getri( int N, T& error );
...
int Lapack<T>::run_all_test( )
{
    ...
    if ( test_copy<T>(N,error) ) { printf("test_copy failed (%e)\n",error); N_errors++; }
    if ( test_nrm2<T>(N,error) ) { printf("test_nrm2 failed (%e)\n",error); N_errors++; }
    if ( test_axpy<T>(N,error) ) { printf("test_axpy failed (%e)\n",error); N_errors++; }
    if ( test_asum<T>(N,error) ) { printf("test_asum failed (%e)\n",error); N_errors++; }
    if ( test_dot<T>(N,error)  ) { printf("test_dot failed (%e)\n",error);  N_errors++; }
    // Matrix blas operations
    if ( test_gemv<T>(N,error) ) { printf("test_gemv failed (%e)\n",error); N_errors++; }
    if ( test_gemm<T>(N,error) ) { printf("test_gemm failed (%e)\n",error); N_errors++; }
    // Linear solves
    if ( test_gesv<T>(N,error) ) { printf("test_gesv failed (%e)\n",error); N_errors++; }
    if ( test_gtsv<T>(N,error) ) { printf("test_gtsv failed (%e)\n",error); N_errors++; }
    if ( test_gbsv<T>(N,error) ) { printf("test_gbsv failed (%e)\n",error); N_errors++; }
    // Linear factorizations
    if ( test_getrf<T>(N,error) ) { printf("test_getrf failed (%e)\n",error); N_errors++; }
    if ( test_gttrf<T>(N,error) ) { printf("test_gttrf failed (%e)\n",error); N_errors++; }
    if ( test_gbtrf<T>(N,error) ) { printf("test_gbtrf failed (%e)\n",error); N_errors++; }
    // Solve using factorization
    if ( test_getrs<T>(N,error) ) { printf("test_getrs failed (%e)\n",error); N_errors++; }
    if ( test_gttrs<T>(N,error) ) { printf("test_gttrs failed (%e)\n",error); N_errors++; }
    if ( test_gbtrs<T>(N,error) ) { printf("test_gbtrs failed (%e)\n",error); N_errors++; }
    // Inverse using factorization
    if ( test_getri<T>(N,error) ) { printf("test_getri failed (%e)\n",error); N_errors++; }
    return N_errors>0;
}
...
static bool test_dot( int N, T& error )
{
    ...
    for (int i=0; i<N; i++) {
        ...
        error = std::max(error,std::abs(ans1-ans2)/K);
        if ( std::abs(ans1-ans2)>2*K*std::numeric_limits<T>::epsilon() )
            N_errors++;
    }
    delete [] x1;
    delete [] x2;
