...
inline void Counter::increment( const std::string &key, size_t amt )
{
    AMP_INSIST( d_Data.find( key ) != d_Data.end(), "Uninitialized key" );
    d_Data[key] += amt;
}

inline size_t Counter::total( const std::string &key, AMP_MPI comm )
{
    AMP_INSIST( d_Data.find( key ) != d_Data.end(), "Uninitialized key" );
...
inline void Counter::increment( const std::string &, size_t ) {}

inline size_t Counter::total( const std::string &, AMP_MPI ) { return 0; }
#endif
}
