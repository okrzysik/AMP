...
void  linearRegressionTest(AMP::UnitTest *ut) {
  ...
  if( fabs(A - MatlabA) > CUSTOM_EPSILON
      || fabs(B - MatlabB) > CUSTOM_EPSILON 
      || fabs(CoefDeterm - MatlabCoefDeterm) > CUSTOM_EPSILON 
      || fabs(CoefCorrel - MatlabCoefCorrel) > CUSTOM_EPSILON 
      || fabs(StdErrorEst - MatlabStdErrorEst) > CUSTOM_EPSILON ) ut->failure(thisTest); 
  else ut->passes(thisTest);
