...
int main( int, char*[] ) 
{
    ...
    if ( N_errors==0 ) { printf("  passed dp tests\n"); } else { printf("failed %d dp tests\n", N_errors); }
    printf("\nRunning single precision basic tests\n");
    N_errors += Lapack<float>::run_all_test();
    if ( N_errors==0 ) { printf("  passed sp tests\n"); } else { printf("failed %d sp tests \n", N_errors); }

    // Get the times for the tests
    printf("\nGetting double precision test times\n");
    const char* dptests[] = { "dcopy", "dscal", "dnrm2", "dasum", "ddot", 
        "daxpy", "dgemv", "dgemm", "dgesv", "dgtsv", "dgbsv", 
        "dgetrf", "dgttrf","dgbtrf", "dgetrs", "dgttrs", "dgbtrs", 
        "dgetri" };
    const int dpN[] = { 500, 500, 500, 500, 500,
    ...
    if ( N_errors==0 ) { printf("  passed dp timing tests\n"); } else { printf("failed %d dp timing tests\n", N_errors); }

    printf("\nGetting single precision test times\n");
    const char* sptests[] = { "scopy", "sscal", "snrm2", "sasum", "sdot", 
        "saxpy", "sgemv", "sgemm", "sgesv", "sgtsv", "sgbsv", 
        "sgetrf", "sgttrf","sgbtrf", "sgetrs", "sgttrs", "sgbtrs", 
        "sgetri" };
    const int spN[] = { 500, 500, 500, 500, 500,
        100, 100, 100, 100, 500, 500,
        100, 500, 500, 100, 500, 500, 
        100 };
    ...
    if ( N_errors==0 ) { printf("  passed sp timing tests\n"); } else { printf("failed %d sp timing tests\n", N_errors); }
