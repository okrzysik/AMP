...
int gpuDeviceInit( int devID )
{
    ...
    if ( device_count == 0 ) {
        fprintf( stderr, "gpuDeviceInit() HIP error: no devices supporting HIP.\n" );
        exit( EXIT_FAILURE );
    }
...
int gpuGetMaxGflopsDeviceId()
{
    ...
    if ( device_count == 0 ) {
        fprintf( stderr, "gpuGetMaxGflopsDeviceId() HIP error: no devices supporting HIP.\n" );
        exit( EXIT_FAILURE );
    }
    ...
    if ( devices_prohibited == device_count ) {
        fprintf(
            stderr,
            "gpuGetMaxGflopsDeviceId() HIP error: all devices have compute mode prohibited.\n" );
        exit( EXIT_FAILURE );
    }
...
bool checkHipCapabilities( int major_version, int minor_version )
{
    ...
    if ( ( deviceProp.major > major_version ) ||
        ...
        printf( "  No GPU device was found that can support HIP compute capability %d.%d.\n",
                major_version,
                minor_version );
        return false;
    }
