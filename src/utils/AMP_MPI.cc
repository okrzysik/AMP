...
{
    if ( error != MPI_SUCCESS ) MPI_ERROR( "Error calling MPI routine" );
}
#endif
...
std::vector<int> MPI_CLASS::getProcessAffinity()
{
    ...
    if ( error != 0 ) MPI_ERROR( "Error getting process affinity" );
    for ( int i = 0; i < (int) sizeof( cpu_set_t ) * CHAR_BIT; i++ ) {
        if ( CPU_ISSET( i, &mask ) ) procs.push_back( i );
    }
    ...
    for ( int i = 0; i < (int) sizeof( size_t ) * CHAR_BIT; i++ ) {
        if ( ( procMask & 0x1 ) != 0 ) procs.push_back( i );
        procMask >>= 1;
    }
#else
#error Unknown OS
...
void MPI_CLASS::setProcessAffinity( std::vector<int> procs )
{
    ...
    cpu_set_t mask;
    CPU_ZERO( &mask );
    for ( size_t i = 0; i < procs.size(); i++ ) CPU_SET( procs[i], &mask );
    int error      = sched_setaffinity( getpid(), sizeof( cpu_set_t ), &mask );
    if ( error != 0 ) MPI_ERROR( "Error setting process affinity" );
#elif defined( USE_MAC )
    // MAC does not support getting or setting the affinity
    NULL_USE( procs );
#elif defined( USE_WINDOWS )
    DWORD mask = 0;
    for ( size_t i = 0; i < procs.size(); i++ ) mask |= ( (DWORD) 1 ) << procs[i];
    HANDLE hProc   = GetCurrentProcess();
                                  ...
                                  const int N_max_in )
{
    ...
    if ( cpus.empty() ) {
        for ( int i = 0; i < getNumberOfProcessors(); i++ ) cpus.push_back( i );
    }
...
void MPI_CLASS::reset()
{
    ...
    int count                    = -1;
    if ( d_count != NULL ) count = decrement_count( d_count );
    if ( count == 0 ) {
// We are holding that last reference to the MPI_Comm object, we need to free it
        ...
        if ( err != MPI_SUCCESS ) MPI_ERROR( "Problem free'ing MPI_Comm object" );
        communicator = MPI_CLASS_COMM_NULL;
...
MPI_CLASS::MPI_CLASS( const MPI_CLASS &comm )
{
    ...
    if ( d_currentTag != NULL ) ++d_currentTag[1];
    ...
    if ( d_count != NULL ) increment_count( d_count );
    tmp_alignment = -1;
}
...
MPI_CLASS &MPI_CLASS::operator=( const MPI_CLASS &comm )
{
    ...
    if ( this->d_currentTag != NULL ) ++( this->d_currentTag[1] );
    // Set and increment the count
    this->d_count = comm.d_count;
    if ( this->d_count != NULL ) increment_count( this->d_count );
...
MPI_CLASS::MPI_CLASS( MPI_Comm comm, bool manage )
{
    ...
    if ( communicator != MPI_COMM_NULL ) {
        ...
        if ( MPI_SIZE_T == 0x0 ) MPI_SIZE_T = getSizeTDataType();
...
MPI_CLASS MPI_CLASS::intersect( const MPI_CLASS &comm1, const MPI_CLASS &comm2 )
{
    if ( comm1.isNull() || comm2.isNull() ) return MPI_CLASS( MPI_CLASS_COMM_NULL, false );
...
MPI_CLASS MPI_CLASS::split( int color, int key ) const
{
    if ( d_isNull ) {
        ...
        if ( color == -1 ) return MPI_CLASS( MPI_CLASS_COMM_NULL );
        return dup();
    }
...
MPI_CLASS MPI_CLASS::splitByNode( int key ) const
{
    ...
    if ( comm_size == 1 ) return this->split( 0, 0 );
    // Get the node name
...
{
    if ( d_isNull ) return MPI_CLASS( MPI_CLASS_COMM_NULL );
    MPI_Comm new_MPI_comm;
#ifdef USE_MPI
...
bool MPI_CLASS::operator<( const MPI_CLASS &comm ) const
{
    ...
    if ( this->d_isNull ) return false;
    if ( comm.d_isNull ) flag = false;
    // Use compare to check if the comms are equal
    if ( compare( comm ) != 0 ) return false;
    // Check that the size of the other communicator is > the current communicator size
    if ( comm_size >= comm.comm_size ) flag = false;
...
// this is < comm iff this group is a subgroup of comm's group
    ...
    if ( !d_isNull ) MPI_Comm_group( communicator, &group1 );
    if ( !comm.d_isNull ) MPI_Comm_group( comm.communicator, &group2 );
    MPI_Group_union( group1, group2, &group12 );
    int compare;
    MPI_Group_compare( group2, group12, &compare );
    if ( compare == MPI_UNEQUAL ) flag = false;
...
bool MPI_CLASS::operator<=( const MPI_CLASS &comm ) const
{
    ...
    if ( comm.getSize() == world_size ) return true;
    if ( getSize() == 1 && !comm.d_isNull ) return true;
#endif
    // Use compare to check if the comms are equal
    if ( compare( comm ) != 0 ) return true;
    // Check that the size of the other communicator is > the current communicator size
    // this is <= comm iff this group is a subgroup of comm's group
    if ( comm_size > comm.comm_size ) flag = false;
...
bool MPI_CLASS::operator>=( const MPI_CLASS &comm ) const
{
    ...
    int world_size = 0;
    MPI_Comm_size( MPI_COMM_WORLD, &world_size );
    if ( getSize() == world_size ) return true;
    if ( comm.getSize() == 1 && !comm.d_isNull ) return true;
#endif
    // Use compare to check if the comms are equal
    if ( compare( comm ) != 0 ) return true;
    // Check that the size of the other communicator is > the current communicator size
...
{
    if ( communicator == comm.communicator ) return 1;
#ifdef USE_MPI
    if ( d_isNull || comm.d_isNull ) return 0;
    int result;
...
// std::complex<double>
template <>
void MPI_CLASS::call_sumReduce<std::complex<double>>( const std::complex<double> *x,
                                                      ...
                                                      const int n ) const
{
    ...
    for ( int i = 0; i < n; i++ ) y[i] = std::complex<double>( recv[2 * i + 0], recv[2 * i + 1] );
    delete[] send;
    delete[] recv;
...
void MPI_CLASS::call_sumReduce<std::complex<double>>( std::complex<double> *x, const int n ) const
{
    ...
    for ( int i = 0; i < n; i++ ) x[i] = std::complex<double>( recv[2 * i + 0], recv[2 * i + 1] );
    delete[] send;
    delete[] recv;
                                               ...
                                               int *comm_rank_of_min ) const
{
    if ( comm_rank_of_min == NULL ) {
        ...
        int *tmp = new int[n];
        for ( int i = 0; i < n; i++ ) tmp[i] = send[i];
        call_minReduce<int>( tmp, n, comm_rank_of_min );
        for ( int i = 0; i < n; i++ ) recv[i] = static_cast<unsigned char>( tmp[i] );
        delete[] tmp;
    }
}
template <>
void MPI_CLASS::call_minReduce<unsigned char>( unsigned char *x,
                                               const int n,
                                               int *comm_rank_of_min ) const
{
    if ( comm_rank_of_min == NULL ) {
        ...
        call_minReduce<int>( tmp, n, comm_rank_of_min );
        for ( int i = 0; i < n; i++ ) x[i] = static_cast<unsigned char>( tmp[i] );
        delete[] tmp;
    }
}
// char
template <>
void MPI_CLASS::call_minReduce<char>( const char *send,
                                      char *recv,
                                      const int n,
                                      int *comm_rank_of_min ) const
{
    if ( comm_rank_of_min == NULL ) {
        ...
        for ( int i = 0; i < n; i++ ) tmp[i] = send[i];
        call_minReduce<int>( tmp, n, comm_rank_of_min );
        for ( int i = 0; i < n; i++ ) recv[i] = static_cast<char>( tmp[i] );
        delete[] tmp;
    }
}
template <>
void MPI_CLASS::call_minReduce<char>( char *x, const int n, int *comm_rank_of_min ) const
{
    if ( comm_rank_of_min == NULL ) {
        ...
        for ( int i = 0; i < n; i++ ) tmp[i] = x[i];
        call_minReduce<int>( tmp, n, comm_rank_of_min );
        for ( int i = 0; i < n; i++ ) x[i] = static_cast<char>( tmp[i] );
        delete[] tmp;
    }
}
// unsigned int
                                              ...
                                              int *comm_rank_of_min ) const
{
    if ( comm_rank_of_min == NULL ) {
        ...
        int *tmp = new int[n];
        for ( int i = 0; i < n; i++ ) tmp[i] = unsigned_to_signed( send[i] );
        call_minReduce<int>( tmp, n, comm_rank_of_min );
        for ( int i = 0; i < n; i++ ) recv[i] = signed_to_unsigned( tmp[i] );
        delete[] tmp;
    }
}
template <>
void MPI_CLASS::call_minReduce<unsigned int>( unsigned int *x,
                                              const int n,
                                              int *comm_rank_of_min ) const
{
    if ( comm_rank_of_min == NULL ) {
        ...
        int *tmp = new int[n];
        for ( int i = 0; i < n; i++ ) tmp[i] = unsigned_to_signed( x[i] );
        call_minReduce<int>( tmp, n, comm_rank_of_min );
        for ( int i = 0; i < n; i++ ) x[i] = signed_to_unsigned( tmp[i] );
        delete[] tmp;
    }
}
// int
template <>
void MPI_CLASS::call_minReduce<int>( const int *x,
                                     int *y,
                                     const int n,
                                     int *comm_rank_of_min ) const
{
...
// long int
template <>
void MPI_CLASS::call_minReduce<long int>( const long int *x,
                                          long int *y,
                                          const int n,
                                          int *comm_rank_of_min ) const
{
...
void MPI_CLASS::call_minReduce<long int>( long int *x, const int n, int *comm_rank_of_min ) const
{
    ...
    if ( comm_rank_of_min == NULL ) {
        ...
        for ( long int i = 0; i < n; i++ ) x[i] = recv[i];
        delete[] recv;
    }
    else {
        LongIntStruct *recv = new LongIntStruct[n];
                                                        ...
                                                        int *comm_rank_of_min ) const
{
    ...
    if ( comm_rank_of_min == NULL ) {
        ...
        for ( int i = 0; i < n; i++ ) x[i] = unsigned_to_signed( send[i] );
        MPI_Allreduce( (void *) x, (void *) y, n, MPI_LONG_LONG_INT, MPI_MIN, communicator );
        for ( int i = 0; i < n; i++ ) recv[i] = signed_to_unsigned( y[i] );
        delete[] x;
        delete[] y;
    }
    else {
        ...
        for ( int i = 0; i < n; i++ ) tmp[i] = static_cast<double>( send[i] );
        call_minReduce<double>( tmp, n, comm_rank_of_min );
        for ( int i = 0; i < n; i++ ) recv[i] = static_cast<long long int>( tmp[i] );
    }
    PROFILE_STOP( "minReduce1<long int>", profile_level );
}
template <>
void MPI_CLASS::call_minReduce<unsigned long long int>( unsigned long long int *x,
                                                        const int n,
                                                        int *comm_rank_of_min ) const
{
                                               ...
                                               int *comm_rank_of_min ) const
{
    ...
    if ( comm_rank_of_min == NULL ) {
        ...
        printf( "minReduce<long long int> will use double\n" );
        double *tmp = new double[n];
        for ( int i = 0; i < n; i++ ) tmp[i] = static_cast<double>( x[i] );
        call_minReduce<double>( tmp, n, comm_rank_of_min );
        for ( int i = 0; i < n; i++ ) y[i] = static_cast<long long int>( tmp[i] );
    }
    PROFILE_STOP( "minReduce1<long int>", profile_level );
}
template <>
void MPI_CLASS::call_minReduce<long long int>( long long int *x,
                                               const int n,
                                               int *comm_rank_of_min ) const
{
    long long int *recv = new long long int[n];
    call_minReduce<long long int>( x, recv, n, comm_rank_of_min );
    for ( int i = 0; i < n; i++ ) x[i] = signed_to_unsigned( recv[i] );
    delete[] recv;
}
// float
template <>
void MPI_CLASS::call_minReduce<float>( const float *x,
                                       float *y,
                                       const int n,
                                       int *comm_rank_of_min ) const
{
...
// double
template <>
void MPI_CLASS::call_minReduce<double>( const double *x,
                                        double *y,
                                        const int n,
                                        int *comm_rank_of_min ) const
{
...
template <>
void MPI_CLASS::call_maxReduce<unsigned char>( unsigned char *x,
                                               const int n,
                                               int *comm_rank_of_max ) const
{
    if ( comm_rank_of_max == NULL ) {
...
// char
template <>
void MPI_CLASS::call_maxReduce<char>( const char *send,
                                      char *recv,
                                      const int n,
                                      int *comm_rank_of_max ) const
{
...
template <>
void MPI_CLASS::call_maxReduce<unsigned int>( unsigned int *x,
                                              const int n,
                                              int *comm_rank_of_max ) const
{
    if ( comm_rank_of_max == NULL ) {
...
// int
template <>
void MPI_CLASS::call_maxReduce<int>( const int *x,
                                     int *y,
                                     const int n,
                                     int *comm_rank_of_max ) const
{
...
// long int
template <>
void MPI_CLASS::call_maxReduce<long int>( const long int *x,
                                          long int *y,
                                          const int n,
                                          int *comm_rank_of_max ) const
{
...
template <>
void MPI_CLASS::call_maxReduce<unsigned long long int>( unsigned long long int *x,
                                                        const int n,
                                                        int *comm_rank_of_max ) const
{
    unsigned long long int *recv = new unsigned long long int[n];
...
template <>
void MPI_CLASS::call_maxReduce<long long int>( long long int *x,
                                               const int n,
                                               int *comm_rank_of_max ) const
{
    long long int *recv = new long long int[n];
...
// float
template <>
void MPI_CLASS::call_maxReduce<float>( const float *x,
                                       float *y,
                                       const int n,
                                       int *comm_rank_of_max ) const
{
...
// double
template <>
void MPI_CLASS::call_maxReduce<double>( const double *x,
                                        double *y,
                                        const int n,
                                        int *comm_rank_of_max ) const
{
...
// int
template <>
void MPI_CLASS::send<int>( const int *buf,
                           const int length,
                           const int recv_proc_number,
                           int tag ) const
{
...
// float
template <>
void MPI_CLASS::send<float>( const float *buf,
                             const int length,
                             const int recv_proc_number,
                             int tag ) const
{
...
// double
template <>
void MPI_CLASS::send<double>( const double *buf,
                              const int length,
                              const int recv_proc_number,
                              int tag ) const
{
...
template <>
MPI_Request
MPI_CLASS::Isend<int>( const int *buf, const int length, const int recv_proc, const int tag ) const
{
    MPI_INSIST( tag <= d_maxTag, "Maximum tag value exceeded" );
    MPI_INSIST( tag >= 0, "tag must be >= 0" );
    MPI_Request request;
...
// float
template <>
MPI_Request MPI_CLASS::Isend<float>( const float *buf,
                                     const int length,
                                     const int recv_proc,
                                     const int tag ) const
{
...
// double
template <>
MPI_Request MPI_CLASS::Isend<double>( const double *buf,
                                      const int length,
                                      const int recv_proc,
                                      const int tag ) const
{
...
************************************************************************/
void MPI_CLASS::sendBytes( const void *buf,
...
************************************************************************/
MPI_Request MPI_CLASS::IsendBytes( const void *buf,
                                   const int number_bytes,
                                   const int recv_proc,
                                   const int tag ) const
{
...
template <>
MPI_Request
MPI_CLASS::Irecv<int>( int *buf, const int length, const int send_proc, const int tag ) const
{
    MPI_INSIST( tag <= d_maxTag, "Maximum tag value exceeded" );
    MPI_INSIST( tag >= 0, "tag must be >= 0" );
    MPI_Request request;
...
template <>
MPI_Request
MPI_CLASS::Irecv<float>( float *buf, const int length, const int send_proc, const int tag ) const
{
    MPI_INSIST( tag <= d_maxTag, "Maximum tag value exceeded" );
    MPI_INSIST( tag >= 0, "tag must be >= 0" );
    MPI_Request request;
...
template <>
MPI_Request
MPI_CLASS::Irecv<double>( double *buf, const int length, const int send_proc, const int tag ) const
{
    MPI_INSIST( tag <= d_maxTag, "Maximum tag value exceeded" );
    MPI_INSIST( tag >= 0, "tag must be >= 0" );
    MPI_Request request;
...
************************************************************************/
MPI_Request
MPI_CLASS::IrecvBytes( void *buf, const int number_bytes, const int send_proc, const int tag ) const
{
    MPI_INSIST( tag <= d_maxTag, "Maximum tag value exceeded" );
    MPI_INSIST( tag >= 0, "tag must be >= 0" );
...
template <>
void MPI_CLASS::allToAll<unsigned int>( const int n,
...
// unsigned char
template <>
void MPI_CLASS::call_allToAll<unsigned char>( const unsigned char *send_data,
                                              const int send_cnt[],
                                              const int send_disp[],
...
// unsigned int
template <>
void MPI_CLASS::call_allToAll<unsigned int>( const unsigned int *send_data,
                                             const int send_cnt[],
                                             const int send_disp[],
...
// int
template <>
void MPI_CLASS::call_allToAll<int>( const int *send_data,
                                    const int send_cnt[],
                                    const int send_disp[],
                                    int *recv_data,
                                    const int *recv_cnt,
...
// long int
template <>
void MPI_CLASS::call_allToAll<long int>( const long int *send_data,
                                         const int send_cnt[],
                                         const int send_disp[],
                                         long int *recv_data,
                                         const int *recv_cnt,
...
// std::complex<double>
template <>
void MPI_CLASS::call_sumScan<std::complex<double>>( const std::complex<double> *x,
                                                    std::complex<double> *y,
                                                    int n ) const
{
    double *send = new double[2 * n];
...
std::vector<int> MPI_CLASS::commRanks( const std::vector<int> &ranks ) const
{
    ...
    for ( size_t i = 0; i < ranks.size(); i++ ) data1[ranks[i]] = 1;
    MPI_Alltoall( data1, 1, MPI_CHAR, data2, 1, MPI_CHAR, communicator );
    int N = 0;
    for ( int i = 0; i < comm_size; i++ ) N += data2[i];
    std::vector<int> ranks_out;
    ranks_out.reserve( N );
    for ( int i = 0; i < comm_size; i++ ) {
        if ( data2[i] ) ranks_out.push_back( i );
    }
    delete[] data1;
    delete[] data2;
    return ranks_out;
...
void MPI_CLASS::wait( MPI_Request request )
{
    ...
    while ( 1 ) {
        ...
        if ( global_isendrecv_list.find( request ) == global_isendrecv_list.end() ) break;
        // Put the current thread to sleep to allow other threads to run
        sched_yield();
    }
    PROFILE_STOP( "wait", profile_level );
