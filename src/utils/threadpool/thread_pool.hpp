...
#define TPOOL_ADD_WORK( TPOOL, FUNCTION, ARGS, ... )                     \
    ThreadPool_add_work( TPOOL,                                          \
                         TPOOL_GET_PRIORITY( 0, __VA_ARGS__, 0, 0 ) + 0, \
                         FUNCTION,                                       \
...
inline Ret apply_helper( std::function<Ret( Args... )> &pf,
                         index_tuple<Indexes...>,
                         std::tuple<Args...> &&tup )
{
...
template<class Ret, class... Args>
inline ThreadPool::thread_id_t
ThreadPool_add_work( ThreadPool *tpool, int priority, Ret ( *routine )( Args... ), Args... args )
{
    auto work = new WorkItemFull<Ret, Args...>( routine, std::forward_as_tuple( args... ) );
    return ThreadPool::add_work( tpool, work, priority );
}
template<class Ret>
inline ThreadPool::thread_id_t
ThreadPool_add_work( ThreadPool *tpool, int priority, Ret ( *routine )(), void * )
{
...
inline void ThreadPool::wait_all( size_t N_work, const ThreadPool::thread_id_t *ids ) const
{
    if ( N_work == 0 )
        return;
    auto finished = new bool[N_work];
...
inline std::vector<int> ThreadPool::wait_some( int N_wait,
                                               const std::vector<thread_id_t> &ids ) const
{
    auto finished  = new bool[ids.size()];
...
inline std::vector<ThreadPool::thread_id_t>
...
inline ThreadPool::thread_id_t &ThreadPool::thread_id_t::
operator=( const ThreadPool::thread_id_t &rhs ) volatile
{
...
inline ThreadPool::thread_id_t &ThreadPool::thread_id_t::
operator=( volatile ThreadPool::thread_id_t &&rhs ) volatile
{
...
inline ThreadPool::thread_id_t &ThreadPool::thread_id_t::
operator=( const ThreadPool::thread_id_t &rhs )
{
    if ( this == &rhs ) // protect against invalid self-assignment
        return const_cast<ThreadPool::thread_id_t &>( *this );
    this->reset();
...
inline ThreadPool::thread_id_t &ThreadPool::thread_id_t::
operator=( const volatile ThreadPool::thread_id_t &rhs )
{
    if ( this == &rhs ) // protect against invalid self-assignment
        return const_cast<ThreadPool::thread_id_t &>( *this );
    this->reset();
...
inline ThreadPool::thread_id_t &ThreadPool::thread_id_t::
operator=( const volatile ThreadPool::thread_id_t &rhs ) volatile
{
    if ( this == &rhs ) // protect against invalid self-assignment
        return const_cast<ThreadPool::thread_id_t &>( *this );
    this->reset();
