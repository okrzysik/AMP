...
static_assert( ThreadPool::MAX_QUEUED < 65535, "MAX_QUEUED must < 65535" );
...
constexpr uint16_t ThreadPool::MAX_QUEUED;
 ...
 ******************************************************************/
ThreadPool::ThreadPool() { initialize( 0, "none", 0, nullptr ); }
ThreadPool::ThreadPool( const int N, const std::string &affinity, const std::vector<int> &procs )
{
...
ThreadPool::~ThreadPool()
{
    ...
    printp( "add_work:  %lu us,  %lu us,  %lu us,  %lu us,  %lu us\n",
            total_add_work_time[0] / 1000,
            total_add_work_time[1] / 1000,
            total_add_work_time[2] / 1000,
            total_add_work_time[3] / 1000,
        ...
        if ( d_queue_list.size() > 0 ) {
            ...
            if ( d_errorHandler ) {
                try {
                    ...
                    auto msg = Utilities::stringf( "Error, caught unknown exception in thread %i\n",
                                                   thread_id );
                    d_errorHandler( msg );
                }
            } else {
                           ...
                           ThreadPool::thread_id_t *ids )
{
    ...
    if ( N > MAX_QUEUED - d_queue_list.size() ) {
        int N_wait = N - ( MAX_QUEUED - d_queue_list.size() );
        while ( N_wait > 0 ) {
            d_signal_count = std::min<int>( N_wait, 255 );
            d_wait_finished.wait_for( 1e-4 );
            N_wait = N - ( MAX_QUEUED - d_queue_list.size() );
        }
    }
#if MONITOR_THREADPOOL_PERFORMANCE == 1
 ...
 ******************************************************************/
static inline void check_finished( size_t N_work,
                                   const ThreadPool::thread_id_t *ids,
                                   size_t &N_finished,
                                   bool *finished )
{
...
int ThreadPool::wait_some( size_t N_work,
                           const ThreadPool::thread_id_t *ids,
                           size_t N_wait,
                           bool *finished ) const
{
...
ThreadPool::wait_ids_struct::wait_ids_struct(
    ...
    const ThreadPool::thread_id_t *ids,
    size_t N_wait,
    AtomicOperations::pool<condition_variable, 128> &cv_pool,
    int N_wait_list,
