...
****************************************************************/
void structuredFaceDOFManager::getDOFs( const AMP::Mesh::MeshElementID &id, std::vector <size_t> &dofs ) const
{
    dofs.resize(0);
    ...
    for (int d=0; d<3; d++) {
        if ( !d_local_ids[d].empty() ) {
            ...
            if ( index == d_local_ids[d].size() ) { index--; }
    ...
    for (int d=0; d<3; d++) {
        if ( !d_remote_ids[d].empty() ) {
            ...
            if ( index == d_remote_ids[d].size() ) { index--; }
...
AMP::Mesh::MeshIterator structuredFaceDOFManager::getIterator( ) const
{
    ...
    for (int i=0; i<3; i++) {
        if ( d_DOFsPerFace[i] > 0 ) 
            faces[i].reset( new AMP::Mesh::MeshIterator(AMP::Mesh::StructuredMeshHelper::getFaceIterator(d_mesh,0,i)) );
    }
