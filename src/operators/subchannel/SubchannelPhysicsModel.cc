...
{
	// get material key
	AMP_INSIST( (params->d_db->keyExists("Material")), "Subchannel Key ''Material'' is missing!" );
	...
	AMP_INSIST( (params->d_db->keyExists("Formulation")), "Subchannel Key ''Formulation'' is missing!" );
	...
	for (std::vector<std::string>::iterator prop = properties.begin(); prop != properties.end(); ++prop){
	...
	if (params->d_db->keyExists("Defaults")) {
		...
		std::map<std::string, bool> defaults_found; // maps Defaults key to boolean for that key being found
		...
		for (std::vector<std::string>::iterator key = defaultkeys.begin(); key != defaultkeys.end(); ++key){
			...
			AMP_INSIST(defaults_found.find(*key)->second, insist_string);
		}
	
		// load and check defaults:
		// for each property needed by formulation
		for (std::vector<std::string>::iterator prop = properties.begin(); prop != properties.end(); ++prop){
			AMP::Materials::PropertyPtr property = d_properties.find(*prop)->second;	// pointer to property
			...
			for (size_t i=0; i<n_arguments; ++i) {
				...
				std::vector<std::string>::iterator hit = std::find(defaultkeys.begin(),defaultkeys.end(),argnames[i]);
				// if found,
