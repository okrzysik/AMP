...
void DiffusionNonlinearElement::apply() {
    ...
    if (d_transportAtGauss) {
        ...
        for (size_t var = 0; var < Diffusion::NUMBER_VARIABLES; var++)
        {
	      ...
	      transport_args.insert(std::make_pair(Diffusion::names[var], boost::shared_ptr<std::vector<double> >
						   (new std::vector<double>(d_qrule->n_points())))); 
                for (size_t qp = 0; qp < d_qrule->n_points(); qp++)
                {
		  (*transport_args[Diffusion::names[var]])[qp] = 0.0;
                    for (size_t j = 0; j < num_nodes; j++)
                    {
		      (*transport_args[Diffusion::names[var]])[qp]
                                += d_elementInputVectors[var][j] * phi[j][qp];
                    }//end for j
                }//end for qp
            }
        }
    ...
    // at nodes
        ...
        for (size_t i=0; i<num_nodes; i++) {elem_nodes[i] = d_elem->point(i);}
    ...
    for (size_t qp = 0; qp < d_qrule->n_points(); qp++) {
        ...
        for (size_t n = 0; n < num_nodes; n++) {
            grad_phi += dphi[n][qp]
                    * d_elementInputVectors[d_PrincipalVariable][n];
        }
