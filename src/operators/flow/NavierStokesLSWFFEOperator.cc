...
void NavierStokesLSWFFEOperator::postElementOperation()
{
    ...
    /*
       ...
       d_outVariables->getVariable(NavierStokes::VELOCITY);
          AMP::LinearAlgebra::Variable::shared_ptr pressureVar        =
       d_outVariables->getVariable(NavierStokes::PRESSURE);
          AMP::LinearAlgebra::Variable::shared_ptr principalStressVar =
       d_outVariables->getVariable(NavierStokes::PRINCIPALSTRESS);
          AMP::LinearAlgebra::Variable::shared_ptr shearStressVar     =
       d_outVariables->getVariable(NavierStokes::SHEARSTRESS);
          ...
          AMP::LinearAlgebra::Vector::shared_ptr d_pstOutVec = mySubsetVector(d_outVec,
       principalStressVar );
          AMP::LinearAlgebra::Vector::shared_ptr d_sstOutVec = mySubsetVector(d_outVec,
       shearStressVar );
...
AMP::shared_ptr<OperatorParameters> NavierStokesLSWFFEOperator::getJacobianParameters(
    AMP::LinearAlgebra::Vector::const_shared_ptr u_in )
{
    ...
    AMP::shared_ptr<NavierStokesLinearFEOperatorParameters> outParams(
        new NavierStokesLinearFEOperatorParameters( tmp_db ) );
    /*
            for(unsigned int i = 0; i < NavierStokes::TOTAL_NUMBER_OF_VARIABLES; i++) {
              if(d_isActive[i]) {
                  AMP::LinearAlgebra::Variable::shared_ptr var = d_inpVariables->getVariable(i);
