...
#include "AMP/operators/flow/NavierStokesGalWFLinearFEOperator.h"
...
/*
   ...
   params->d_db->getWithDefault<bool>("isNonlinearOperatorInitialized",
   false);
          if(isNonlinearOperatorInitialized) {
            reset(params);
          } else {
            AMP::LinearAlgebra::Vector::shared_ptr tmpInVec =
   AMP::LinearAlgebra::createVector(d_inDofMap,
   d_inpVariable, true);
            AMP::LinearAlgebra::Vector::shared_ptr tmpOutVec =
   AMP::LinearAlgebra::createVector(d_outDofMap,
   d_outVariable, true);
   ...
   NULL)) {
        d_inVec[NavierStokes::VELOCITY] =
   params->d_frozenVec[NavierStokes::VELOCITY]->cloneVector();
      }
      ...
      if((d_inVec[NavierStokes::PRESSURE].get() == NULL) and
   (params->d_frozenVec[NavierStokes::PRESSURE].get() !=
   NULL)) {
        d_inVec[NavierStokes::PRESSURE] =
   params->d_frozenVec[NavierStokes::PRESSURE]->cloneVector();
      }
      ...
      if(d_inVec[NavierStokes::TEMPERATURE].get() == NULL and
   params->d_frozenVec[NavierStokes::TEMPERATURE].get() !=
   NULL) {
        d_inVec[NavierStokes::TEMPERATURE] =
   params->d_frozenVec[NavierStokes::TEMPERATURE]->cloneVector();
      }
   ...
   (d_inVec[NavierStokes::TEMPERATURE])->getValueByGlobalID(
   d_type1DofIndices[r] );
        }
      }
    ...
    void NavierStokesGalWFLinearFEOperator :: gettype0DofIndicesForCurrentElement(int varId,
   std::vector<std::vector<size_t> > & dofIds) {
    ...
    void NavierStokesGalWFLinearFEOperator :: gettype1DofIndicesForCurrentElement(int varId,
   std::vector<size_t> &
   dofIds) {
      for(unsigned int j = 0; j < d_currNodes.size(); j++) {
