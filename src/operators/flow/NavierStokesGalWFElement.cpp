...
void NavierStokesGalWFElement::initTransportModel() { d_fe->reinit( d_elem ); }
...
void NavierStokesGalWFElement::apply()
{
    ...
    for ( unsigned int qp = 0; qp < d_qrule->n_points(); qp++ ) {
        ...
        for ( unsigned int k = 0; k < num_nodes; k++ ) {
            ...
            dudx +=
                ( elementInputVectors[NavierStokes::VELOCITY][( 3 * k ) + 0] * dphi[k][qp]( 0 ) );
            dudy +=
                ( elementInputVectors[NavierStokes::VELOCITY][( 3 * k ) + 0] * dphi[k][qp]( 1 ) );
            dudz +=
                ( elementInputVectors[NavierStokes::VELOCITY][( 3 * k ) + 0] * dphi[k][qp]( 2 ) );

            dvdx +=
                ( elementInputVectors[NavierStokes::VELOCITY][( 3 * k ) + 1] * dphi[k][qp]( 0 ) );
            dvdy +=
                ( elementInputVectors[NavierStokes::VELOCITY][( 3 * k ) + 1] * dphi[k][qp]( 1 ) );
            dvdz +=
                ( elementInputVectors[NavierStokes::VELOCITY][( 3 * k ) + 1] * dphi[k][qp]( 2 ) );

            dwdx +=
                ( elementInputVectors[NavierStokes::VELOCITY][( 3 * k ) + 2] * dphi[k][qp]( 0 ) );
            dwdy +=
                ( elementInputVectors[NavierStokes::VELOCITY][( 3 * k ) + 2] * dphi[k][qp]( 1 ) );
            dwdz +=
                ( elementInputVectors[NavierStokes::VELOCITY][( 3 * k ) + 2] * dphi[k][qp]( 2 ) );
