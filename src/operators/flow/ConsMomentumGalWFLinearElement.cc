...
namespace Operator {
  ...
  void ConsMomentumGalWFLinearElement :: apply() 
  {
    ...
    for(unsigned int qp = 0; qp < (d_qrule[0])->n_points(); qp++) {
      ...
      for(unsigned int i = 0; i < num_nodes; i++) {
          ...
          for(unsigned int j = 0; j < num_nodes; j++) {
            ...
            elementStiffnessMatrix[3*i    ][3*j     ] += JxW[qp]*( (dphi[i][qp]*dphi[j][qp])  +                                            // diffusion term
                                                                   (u*dphi[j][qp](0)+ v*dphi[j][qp](1)+w*dphi[j][qp](2))*phi[i][qp] +      // convection term
                                                                    dudx*phi[i][qp]*phi[j][qp]   );                                        // Newton term

            elementStiffnessMatrix[3*i    ][3*j + 1 ] += JxW[qp]*dudy*phi[i][qp]*phi[j][qp];                                               // Newton term
            
            elementStiffnessMatrix[3*i    ][3*j + 2 ] += JxW[qp]*dudz*phi[i][qp]*phi[j][qp];                                               // Newton term

            elementStiffnessMatrix[3*i + 1][3*j + 1 ] += JxW[qp]*( (dphi[i][qp]*dphi[j][qp]) +                                             // diffusion term
                                                                   (u*dphi[j][qp](0)+ v*dphi[j][qp](1)+w*dphi[j][qp](2))*phi[i][qp] +      // convection term
                                                                    dvdy*phi[i][qp]*phi[j][qp]);                                           // Newton term

            elementStiffnessMatrix[3*i + 1][3*j     ] += JxW[qp]*dvdx*phi[i][qp]*phi[j][qp];                                               // Newton term
            
            elementStiffnessMatrix[3*i + 1][3*j + 2 ] += JxW[qp]*dvdz*phi[i][qp]*phi[j][qp];                                               // Newton term

            elementStiffnessMatrix[3*i + 2][3*j + 2 ] += JxW[qp]*( (dphi[i][qp]*dphi[j][qp]) +                                             // diffusion term
                                                                   (u*dphi[j][qp](0)+ v*dphi[j][qp](1)+w*dphi[j][qp](2))*phi[i][qp] +      // convection term
                                                                    dwdz*phi[i][qp]*phi[j][qp]);                                           // Newton term

            elementStiffnessMatrix[3*i + 2][3*j     ] += JxW[qp]*dwdx*phi[i][qp]*phi[j][qp];                                               // Newton term
            
            elementStiffnessMatrix[3*i + 2][3*j + 1 ] += JxW[qp]*dwdy*phi[i][qp]*phi[j][qp];                                               // Newton term
