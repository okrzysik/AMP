...
*/        
      ...
      for(unsigned int i = 0; i < num_nodes; i++) {
        ...
        derror_i[1][1] = (phi[i][qp]*dudx + u *dphi[i][qp](0) + v *dphi[i][qp](1) + w *dphi[i][qp](2));
        ...
        derror_i[2][2] = (phi[i][qp]*dvdy + v *dphi[i][qp](1) + u *dphi[i][qp](0) + w *dphi[i][qp](2));
        ...
        derror_i[3][3] = (phi[i][qp]*dwdz + w *dphi[i][qp](2) + u *dphi[i][qp](0) + v *dphi[i][qp](1)); 
        ...
        for(unsigned int j = 0; j < num_nodes; j++) {
          ...
          derror_j[1][1] = (phi[j][qp]*dudx + u *dphi[j][qp](0) + v *dphi[j][qp](1) + w *dphi[j][qp](2));
          ...
          derror_j[2][2] = (phi[j][qp]*dvdy + v *dphi[j][qp](1) + u *dphi[j][qp](0) + w *dphi[j][qp](2));
          ...
          derror_j[3][3] = (phi[j][qp]*dwdz + w *dphi[j][qp](2) + u *dphi[j][qp](0) + v *dphi[j][qp](1)); 
          ...
          for(unsigned int ii = 0; ii < 10 ; ii++) {
            ...
            for(unsigned int jj = 0; jj < 10 ; jj++) {
              ...
              for(unsigned int eq = 0; eq < 10 ; eq++)
                elementStiffnessMatrix[10*i + ii ][10*j + jj  ] += JxW[qp]*( derror_i[eq][ii]*derror_j[eq][jj] ); 
