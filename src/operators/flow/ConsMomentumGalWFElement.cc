...
namespace Operator {
  ...
  void ConsMomentumGalWFElement :: initTransportModel() 
  {
    (d_fe[0])->reinit(d_elem);
  }

  void ConsMomentumGalWFElement :: apply() 
  {
    ...
    for(unsigned int qp = 0; qp < (d_qrule[0])->n_points(); qp++) {
      ...
      for(unsigned int k = 0; k < num_nodes; k++) {
        p += elementInputVectors[NavierStokes::PRESSURE][k]*u_phi[k][qp]; 
      }
      ...
      for(unsigned int j = 0; j < num_nodes; j++) {
        ...
        elementOutputVector[(3*j)     ] += JxW[qp]*( d_alpha_conv * ( u*dudx + v*dudy + w*dudz ) * u_phi[j][qp] + d_density * p * u_dphi[j][qp](0) + 
           d_alpha_diff * d_fmu *( dudx * u_dphi[j][qp](0) + dudy * u_dphi[j][qp](1) + dudz * u_dphi[j][qp](2) ) );

        elementOutputVector[(3*j) + 1 ] += JxW[qp]*( d_alpha_conv * ( u*dvdx + v*dvdy + w*dvdz ) * u_phi[j][qp] + d_density * p * u_dphi[j][qp](1) + 
           d_alpha_diff * d_fmu *( dvdx * u_dphi[j][qp](0) + dvdy * u_dphi[j][qp](1) + dvdz * u_dphi[j][qp](2) ) );

        elementOutputVector[(3*j) + 2 ] += JxW[qp]*( d_alpha_conv * ( u*dwdx + v*dwdy + w*dwdz ) * u_phi[j][qp] + d_density * p * u_dphi[j][qp](2) + 
