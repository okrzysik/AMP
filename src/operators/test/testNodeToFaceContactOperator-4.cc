...
void myTest( AMP::UnitTest *ut, std::string exeName )
{
    ...
    if ( bis ) {
        if ( !useLevitatingFuel ) {
            ...
            makeConstraintsOnFuel( masterMeshAdapter->getBoundaryIDIterator( AMP::Mesh::Vertex, 1 ),
                                   fuelOuterRadius,
                                   masterConstraints,
                                   true );
            makeConstraintsOnFuel( masterMeshAdapter->getBoundaryIDIterator( AMP::Mesh::Vertex, 2 ),
                                   fuelOuterRadius,
                                   masterConstraints,
                                   false );
            makeConstraintsOnFuel( slaveMeshAdapter->getBoundaryIDIterator( AMP::Mesh::Vertex, 2 ),
                                   fuelOuterRadius,
                                   slaveConstraints,
                                   true );
            makeConstraintsOnFuel( slaveMeshAdapter->getBoundaryIDIterator( AMP::Mesh::Vertex, 1 ),
                                   fuelOuterRadius,
                                   slaveConstraints,
                                   false );
        }
        ...
        if ( !cladExpansionConstrained ) {
            ...
            makeConstraintsOnClad( slaveMeshAdapter->getIterator( AMP::Mesh::Vertex ),
                                   cladInnerRadius,
                                   cladOuterRadius,
                                   cladHeight,
                                   slaveConstraints );
    ...
    if ( bis && useALittleHelp ) {
        ...
        AMP::Mesh::MeshIterator it =
            slaveMeshAdapter->getBoundaryIDIterator( AMP::Mesh::Vertex, 2 );
