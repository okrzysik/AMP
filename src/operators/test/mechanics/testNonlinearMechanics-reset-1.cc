...
void myTest(AMP::UnitTest *ut)
{
  ...
  AMP::shared_ptr<AMP::Operator::VonMisesElastoPlasticModel> matModel (new AMP::Operator::VonMisesElastoPlasticModel(matModelParams));
  ...
  for(int useReduced = 0; useReduced < 2; useReduced++) {
    ...
    AMP::shared_ptr<AMP::Database> nonLinElemOp_db = input_db->getDatabase(mechNonlinElemDbStr);
    AMP::shared_ptr<AMP::Operator::ElementOperationParameters> nonLinElemOpParams (new AMP::Operator::ElementOperationParameters( nonLinElemOp_db ));
    AMP::shared_ptr<AMP::Operator::MechanicsNonlinearElement> mechNonlinElem (new AMP::Operator::MechanicsNonlinearElement( nonLinElemOpParams ));

    AMP_INSIST(input_db->keyExists("Mechanics_Nonlinear_Assembly"), "Key ''Mechanics_Nonlinear_Assembly'' is missing!");
    AMP::shared_ptr<AMP::Database> mechNonlinAssembly_db = input_db->getDatabase("Mechanics_Nonlinear_Assembly");
    AMP::shared_ptr<AMP::Operator::MechanicsNonlinearFEOperatorParameters> mechNonlinOpParams(new
    ...
    AMP::shared_ptr<AMP::Operator::MechanicsNonlinearFEOperator> mechNonlinOp (new AMP::Operator::MechanicsNonlinearFEOperator( mechNonlinOpParams ));
    ...
    AMP::shared_ptr<AMP::Operator::ElementOperationParameters> linElemOpParams (new AMP::Operator::ElementOperationParameters( linElemOp_db ));
    AMP::shared_ptr<AMP::Operator::MechanicsLinearElement> mechLinElem (new AMP::Operator::MechanicsLinearElement( linElemOpParams ));

    AMP_INSIST(input_db->keyExists("Mechanics_Linear_Assembly"), "Key ''Mechanics_Linear_Assembly'' is missing!");
    AMP::shared_ptr<AMP::Database> mechLinAssembly_db = input_db->getDatabase("Mechanics_Linear_Assembly");
    AMP::shared_ptr<AMP::Operator::MechanicsLinearFEOperatorParameters> mechLinOpParams(new
    ...
    AMP::shared_ptr<AMP::Operator::DirichletVectorCorrectionParameters> dirichletDispInVecParams(new 
    ...
    AMP::shared_ptr<AMP::Operator::DirichletVectorCorrection> dirichletDispInVecOp(new 
        AMP::Operator::DirichletVectorCorrection(dirichletDispInVecParams));

    AMP::shared_ptr<AMP::Operator::DirichletVectorCorrectionParameters> dirichletDispOutVecParams(new 
        AMP::Operator::DirichletVectorCorrectionParameters(disp_db));
    //This has an in-place apply. So, it has an empty input variable and
    //the output variable is the same as what it is operating on. 
    dirichletDispOutVecParams->d_variable = var;
    dirichletDispOutVecParams->d_Mesh = meshAdapter;
    AMP::shared_ptr<AMP::Operator::DirichletVectorCorrection> dirichletDispOutVecOp (new 
    ...
    AMP::shared_ptr<AMP::Operator::BVPOperatorParameters> linBvpOperatorParams (new AMP::Operator::BVPOperatorParameters(linBvpOp_db));
    linBvpOperatorParams->d_volumeOperator = mechLinOp;
    linBvpOperatorParams->d_boundaryOperator = dirichletMatOp;
    AMP::shared_ptr<AMP::Operator::LinearBVPOperator> linBvpOperator (new AMP::Operator::LinearBVPOperator(linBvpOperatorParams));

    AMP_INSIST(input_db->keyExists("NonlinearBVPOperator"), "Key ''NonlinearBVPOperator'' is missing!");
    AMP::shared_ptr<AMP::Database> nonlinBvpOp_db = input_db->getDatabase("NonlinearBVPOperator");
    AMP::shared_ptr<AMP::Operator::BVPOperatorParameters> nonlinBvpOperatorParams (new AMP::Operator::BVPOperatorParameters(nonlinBvpOp_db));
    nonlinBvpOperatorParams->d_volumeOperator = mechNonlinOp;
    nonlinBvpOperatorParams->d_boundaryOperator = dirichletDispOutVecOp;
    AMP::shared_ptr<AMP::Operator::NonlinearBVPOperator> nonlinBvpOperator (new AMP::Operator::NonlinearBVPOperator(nonlinBvpOperatorParams));
    ...
    AMP::shared_ptr<AMP::Operator::DirichletVectorCorrectionParameters> dirichletLoadVecParams (new 
    ...
    AMP::shared_ptr<AMP::Operator::DirichletVectorCorrection> dirichletLoadVecOp (new 
