...
void myTest(AMP::UnitTest *ut)
{
    ...
    AMP_ASSERT( mesh->getComm().getRank() == 
		dtk_entity_set->communicator()->getRank() );    
    AMP_ASSERT( mesh->getComm().getSize() == 
		dtk_entity_set->communicator()->getSize() );    
	...
	// Check with the iterator.
	{
	    ...
	    dtk_entity_set->getAdjacentEntities( *dtk_iterator,
	...
	// Now get the same entity from the mesh and check again.
	{
	    ...
	    dtk_entity_set->getEntity( DataTransferKit::ENTITY_TYPE_VOLUME,
				       dtk_iterator->id(),
				       dtk_entity );
