...
void nonlinearTest(AMP::UnitTest *ut, std::string exeName)
{
  ...
  subchannelPhysicsModel->getProperty("SaturatedLiquidEnthalpy",liquidEnthalpyResult,liquidEnthalpyArgs);
  ...
  subchannelPhysicsModel->getProperty("ThermalConductivity",conductivityResult,conductivityArgs);
  ...
  for (unsigned int i=0; i<n; ++i) {passed = passed and areEqual(temperatureResult[i],temperatureKnown[i],"temperature");}
  double liquidEnthalpyKnown[n] = {762.683e3,1610.15e3,289.229e3};
  for (unsigned int i=0; i<n; ++i) {passed = passed and areEqual(liquidEnthalpyResult[i],liquidEnthalpyKnown[i],"liquidEnthalpy");}
  double volumeKnown[n] = {0.00105925,0.00119519,0.00100259};
  for (unsigned int i=0; i<n; ++i) {passed = passed and areEqual(volumeResult[i],volumeKnown[i],"volume");}
  double conductivityKnown[n] = {0.684097,0.503998,0.610291};
  for (unsigned int i=0; i<n; ++i) {passed = passed and areEqual(conductivityResult[i],conductivityKnown[i],"conductivity");}
  double viscosityKnown[n] = {0.000218794,7.72970e-5,0.000853838};
  for (unsigned int i=0; i<n; ++i) {passed = passed and areEqual(viscosityResult[i],viscosityKnown[i],"viscosity");}
  double enthalpyKnown[n] = {500.0e3,1.0e6,100.0e3};
  for (unsigned int i=0; i<n; ++i) {passed = passed and areEqual(enthalpyResult[i],enthalpyKnown[i],"enthalpy");}

  // determine if test passed or failed
  if (passed) ut->passes(exeName+": function evaluations");
  else ut->failure(exeName+": function evaluations");
  ...
  subchannelPhysicsModel->getProperty("SaturatedLiquidEnthalpy",liquidEnthalpyResult0Arg,liquidEnthalpyArgs0);
  ...
  subchannelPhysicsModel->getProperty("ThermalConductivity",conductivityResult1Arg,conductivityArgs1);
  std::vector<double> conductivityResult0Arg(1);
  subchannelPhysicsModel->getProperty("ThermalConductivity",conductivityResult0Arg,conductivityArgs0);
  ...
  passed = passed and areEqual(temperatureResult1Arg[0], temperatureKnown1Arg, "temperature, 1 arg");
  double temperatureKnown0Arg = 320.835;
  passed = passed and areEqual(temperatureResult0Arg[0], temperatureKnown0Arg, "temperature, 0 arg");
  double liquidEnthalpyKnown0Arg = 640.185e3;
  passed = passed and areEqual(liquidEnthalpyResult0Arg[0], liquidEnthalpyKnown0Arg, "liquidEnthalpy, 0 arg");
  ...
  passed = passed and areEqual(conductivityResult1Arg[0], conductivityKnown1Arg, "conductivity, 1 arg");
  double conductivityKnown0Arg = 0.668247;
  passed = passed and areEqual(conductivityResult0Arg[0], conductivityKnown0Arg, "conductivity, 0 arg");
  ...
  // determine if test passed or failed
  if (passed) ut->passes(exeName+": defaults evaluations");
  else ut->failure(exeName+": defaults evaluations");
