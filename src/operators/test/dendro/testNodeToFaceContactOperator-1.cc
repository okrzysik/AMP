...
static void myTest( AMP::UnitTest *ut, const std::string &exeName )
{
    ...
    if ( useML && matrixFree ) {
        std::shared_ptr<AMP::Operator::ColumnOperator> dummyColumnOperator(
            new AMP::Operator::ColumnOperator( emptyParams ) );
        dummyColumnOperator->append( masterBVPOperator );
        dummyColumnOperator->append( slaveBVPOperator );
        std::shared_ptr<AMP::Operator::TrilinosMatrixShellOperator> trilinosMatrixShellOperator(
        ...
        std::shared_ptr<AMP::Database> trilinosMLSolver_db =
            columnPreconditioner_db->getDatabase( "MLSolver" );
        trilinosMLSolver_db->putScalar( "USE_EPETRA", false );
        std::shared_ptr<AMP::Solver::TrilinosMLSolverParameters> mlSolverParams(
