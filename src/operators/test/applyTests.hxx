    ...
    AMP::LinearAlgebra::Vector::shared_ptr resVec, const double *shift, const double *scale, const size_t nshift)
{
  ...
  //     Alternatively, one could specify test ranges in the unit test input file and pass into this
  //     routine, but all the input files would have to be modified.
  //     Alternatively, one could specify test ranges in the calling program, but this is less desirable
  //     as many unit test authors do not know ahead of time what material or operator is requested in the
  //     input file.
  ...
  if (false) {
    ...
    if (not passed) {
      ut->failure(msgPrefix + " : apply with random negative values in u, random positive f,r, a=1, b=-1.0");
    }
  }
