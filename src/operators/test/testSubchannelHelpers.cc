...
void testSubchannelHelpers( AMP::UnitTest* ut, std::string input_file ) {
    ...
    double diam = 0.00822;
    double Q_tot = 66.5e3;
    std::vector<double> flux1 = AMP::Operator::Subchannel::getHeatFluxGeneration( "Sinusoidal", z, diam, Q_tot );
    std::vector<double> flux2 = AMP::Operator::Subchannel::getHeatFluxGeneration( "Flat", z, diam, Q_tot );
    double Q_tot1 = 0.0;
    double Q_tot2 = 0.0;
    bool pass_sine = true;
    bool pass_flat = true;
    ...
    for (size_t i=0; i<N_subchannels; i++) {
        ...
        AMP::Mesh::MeshIterator it = AMP::Mesh::StructuredMeshHelper::getXYFaceIterator(subchannelMesh,0);
