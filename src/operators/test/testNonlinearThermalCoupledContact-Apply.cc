    ...
    AMP::LinearAlgebra::Vector::shared_ptr resVec, AMP::LinearAlgebra::Vector::shared_ptr workVec)
{
  ...
  if (passed) {
    ut.passes(msgPrefix + " : apply with u NULL, random values in the vectors f,r, a=1, b=-1.0");
  } else {
    ut.numFails++;
  ...
  if (passed) {
    ut.passes(msgPrefix + " : apply with r NULL, random values in the vectors f,u, a=1, b=-1.0");
  } else {
    ut.numFails++;
  ...
  if (passed) {
    ut.passes(msgPrefix + " : apply with f NULL, u NULL random values in the vector r, a=1, b=-1.0");
  } else {
    ut.numFails++;
  ...
  if (passed) {
    ut.passes(msgPrefix + " : apply with u NULL, r NULL, random values in the vector f, a=1, b=-1.0");
  } else {
    ut.numFails++;
  ...
  if (passed) {
    ut.passes(msgPrefix + " : apply with f, r NULL, random values in the vector u, a=1, b=-1.0");
  } else {
    ut.numFails++;
...
void thermalContactApplyTest(AMP::UnitTest *ut, std::string exeName )
{
...
//-------------------------------------
  ...
  AMP::shared_ptr<AMP::Operator::Operator> testOperator = AMP::dynamic_pointer_cast<AMP::Operator::Operator>(nonlinearCoupledOperator);
  applyTest(ut, msgPrefix, testOperator, RightHandSideVec, TemperatureInKelvin, ResidualVec, WorkVec);
  ut.passes(msgPrefix);
