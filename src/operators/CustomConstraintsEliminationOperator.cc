...
namespace AMP {
  namespace Operator {
    ...
    void CustomConstraintsEliminationOperator::addSlaveToMaster(AMP::LinearAlgebra::Vector::shared_ptr u)
    {
      if (!d_MasterIndices.empty()) {
        ...
        for (size_t i = 0; i < d_SlaveIndices.size(); ++i) {
          if (!d_MasterIndices[i].empty()) {
            ...
            u->addValuesByGlobalID(d_MasterIndices[i].size(), &(d_MasterIndices[i][0]), &(addToMasterValues[0]));
          } // end if
        } // end for i
      } // end if
    }

    void CustomConstraintsEliminationOperator::copyMasterToSlave(AMP::LinearAlgebra::Vector::shared_ptr u)
    {
        ...
        if (!d_MasterIndices.empty()) {
          ...
          for (size_t i = 0; i < d_SlaveIndices.size(); ++i) {
            if (!d_MasterIndices[i].empty()) {
              ...
              u->getValuesByGlobalID(d_MasterIndices[i].size(), &(d_MasterIndices[i][0]), &(masterValues[0]));
              for (size_t j = 0; j < d_MasterIndices[i].size(); ++j) {
    ...
    void CustomConstraintsEliminationOperator::initialize(std::vector<size_t> const & slaveIndices, 
