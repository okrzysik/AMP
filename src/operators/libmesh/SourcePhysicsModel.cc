    ...
    ElementPhysicsModel(params)
{
    ...
    d_elementPhysicsParams.reset(new AMP::Operator::ElementPhysicsModelParameters( params->d_db ));
    ...
    const std::vector<libMesh::Point> & Coordinates )
{
    ...
    if ( d_physicsName=="DiffusionTransportModel" ) {
        ...
        if ( tempVec.size() != result.size() ) {
            ...
            std::vector<double>& burnupVec = *(d_inputMaterialParameters.find("burnup")->second);
            std::vector<double>& oxygenVec = *(d_inputMaterialParameters.find("concentration")->second);
