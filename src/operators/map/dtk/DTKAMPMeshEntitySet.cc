...
#ifdef USE_EXT_MPI
    return Teuchos::rcp( 
	new Teuchos::MpiComm<int>(d_amp_mesh->getComm().getCommunicator()) );
#else
...
int AMPMeshEntitySet::physicalDimension() const
{
    return d_amp_mesh->getDim();
}
				  ...
				  DataTransferKit::Entity& entity ) const
{
    ...
    AMP::Mesh::MeshElementID element_id( 
	is_local, type_id, local_id, owner_rank, mesh_id );
...
AMP::Mesh::GeomType AMPMeshEntitySet::getGeomTypeFromEntityType(
    const DataTransferKit::EntityType entity_type ) const
{
