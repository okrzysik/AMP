...
pullTpetraVectorFromAMPVector( 
    ...
    Teuchos::RCP<const Tpetra::Map<int,std::size_t> > tpetra_map )
{
    // Create a Tpetra map.
    if ( Teuchos::is_null(tpetra_map) )
    {
	AMP::shared_ptr<AMP::Discretization::DOFManager> dofManager = 
	    ampVector->getDOFManager();
    ...
    const AMP::shared_ptr<AMP::Discretization::DOFManager> dof_manager )
{
    ...
    Teuchos::RCP<const Teuchos::Comm<int> > comm =
        Teuchos::rcp( new Teuchos::MpiComm<int>(
			  dof_manager->getComm().getCommunicator()) );
