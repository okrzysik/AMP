    ...
    : d_mapOnThisProc( false ), d_dtk_operator( nullptr )
{
    ...
    DataTransferKit::PredicateFunction select_all = [=]( DataTransferKit::Entity e ) {
        return true;
    };
                            ...
                            AMP::LinearAlgebra::Vector::shared_ptr r )
{
    if ( d_mapOnThisProc ) {
        ...
        Teuchos::RCP<DataTransferKit::Field> r_field = Teuchos::rcp( new DTKAMPField( r ) );

        // Create vectors from the field.
        Teuchos::RCP<DataTransferKit::FieldMultiVector::Base> u_vector =
            Teuchos::rcp( new DataTransferKit::FieldMultiVector( d_TeuchosComm, u_field ) );
        Teuchos::RCP<DataTransferKit::FieldMultiVector::Base> r_vector =
            Teuchos::rcp( new DataTransferKit::FieldMultiVector( d_TeuchosComm, r_field ) );
        ...
        if ( !d_dtk_operator ) {
            ...
            if ( Teuchos::nonnull( d_range_mesh->functionSpace()->entitySet() ) ) {
                ...
                DataTransferKit::LocalEntityPredicate pred(
                    d_range_mesh->functionSpace()->entitySet()->communicator()->getRank() );
                fout << " Iterator size with predicate "
                     << d_range_mesh->functionSpace()->entitySet()->entityIterator( 0, pred ).size()
                     << std::endl;
                fout.close();
            }

            // Setup the operator.
            d_dtk_operator->setup( d_domain_mesh->functionSpace(), d_range_mesh->functionSpace() );

            // get missed range entities
            // std::cout<<" Rank "<<d_comm.getRank()<< " Missed Entities
            // "<<std::dynamic_pointer_cast<DataTransferKit::ConsistentInterpolationOperator>(d_dtk_operator)->getMissedRangeEntityIds()<<std::endl;
        }
