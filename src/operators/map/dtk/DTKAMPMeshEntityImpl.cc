...
DataTransferKit::EntityType AMPMeshEntityImpl::entityType() const
{
	...
	case Mesh::Volume:
	    ...
	    break;
	default:
	    AMP_INSIST( geom_type == Mesh::Vertex ||
			geom_type == Mesh::Edge   ||
			geom_type == Mesh::Face   ||
			geom_type == Mesh::Volume,
 ...
 */
DataTransferKit::EntityId AMPMeshEntityImpl::id() const
{
    return d_id;
}
...
int AMPMeshEntityImpl::physicalDimension() const
{
	...
	d_extra_data->d_element.getElements( Mesh::Vertex );
...
AMPMeshEntityImpl::extraData() const
{
    return d_extra_data;
}
