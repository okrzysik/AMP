        ...
        for( ; bnd != end_bnd; ++bnd) {
          ...
          for(unsigned int i = 0 ; i < numPoints; i++) {
            ...
            if( cur_gauss  >= z[0] && cur_gauss  <= z[pickId]) {
              ...
              mapValues[i]  += ((inputVec)->getValueByGlobalID(ids[originalGaussOrder[0]]) * (z[pickId]-cur_gauss) + (inputVec)->getValueByGlobalID(ids[originalGaussOrder[pickId]])* (cur_gauss-z[0]))/(z[pickId]-z[0]);  
