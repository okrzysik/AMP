    ...
    AMP::LinearAlgebra::Vector::shared_ptr, const double, const double )
{
        ...
        for( ; bnd != end_bnd; ++bnd) {
            ...
            for(unsigned int i = 0 ; i < numPoints; i++) {
                ...
                if( cur_gauss  >= z[0] && cur_gauss  <= z[pickId]) {
                    mapValues[i]  += ((inputVec)->getValueByGlobalID(ids[originalGaussOrder[0]]) * (z[pickId]-cur_gauss) + 
                        (inputVec)->getValueByGlobalID(ids[originalGaussOrder[pickId]])* (cur_gauss-z[0]))/(z[pickId]-z[0]);  
                    numFaceGauss[i] += 1;
                }
            }//end for i
        }//end for bnd
    ...
    // Store the results
    for(unsigned int i = 0 ; i < numPoints; i++) {
        outputVec->setValueByLocalID ( i , aggMapValues[i]/static_cast<double>(aggNumFaceGauss[i]) );
    }
        ...
        for( ; bnd != end_bnd; ++bnd) {
            ...
            for(unsigned int i = 0 ; i < numPoints; i++) {
                ...
                if( cur_node >= z[0] && cur_node <= z[pickId]) {
                    ...
                    mapValues[i]  += ((inputVec)->getValueByGlobalID(dof1[0]) * (z[pickId]-cur_node) + 
                        (inputVec)->getValueByGlobalID(dof2[0])* (cur_node-z[0]))/(z[pickId]-z[0]);  
