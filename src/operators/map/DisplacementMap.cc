...
#include "operators/map/DisplacementMap.h"
#include "operators/map/DisplacementMapParameters.h"


double AMP::Operator::DisplacementMap::Point::_precision;

namespace AMP {
namespace Operator {

  bool DisplacementMap::validMapType ( const std::string &t )
  {
  ...
  void DisplacementMap::sendSurface ( params_shared_ptr p )
  {
    clearRequests ();
...
/*
  ...
  void DisplacementMap::sendOrder ( params_shared_ptr p )
  {
    waitForAllRequests ();
    clearRequests ();
...
*/
  ...
  bool DisplacementMap::continueAsynchronousConstruction ( const boost::shared_ptr < AMP::Operator::OperatorParameters > &params )
  {
    params_shared_ptr p = boost::dynamic_pointer_cast<DisplacementMapParameters> ( params );
    AMP_ASSERT ( p );
  ...
  DisplacementMap::Point::Point ()
  {
  }

  DisplacementMap::Point::Point ( const Point &rhs )
  {
  ...
  bool DisplacementMap::Point::operator == ( const Point &rhs ) const
  {
    double dist = 0.0;
    for ( size_t i = 0 ; i != 3 ; i++ )
    {
...
/*
    ...
    AMP_ASSERT ( params );
    DisplacementMapParameters &Params =
                        *(boost::dynamic_pointer_cast<DisplacementMapParameters> ( params ) );
...
*/
  ...
  void  DisplacementMap::setVector ( AMP::LinearAlgebra::Vector::shared_ptr &p )
  {
    d_OutputVector = p->subsetVectorForVariable ( d_inpVariable );
    AMP_INSIST ( d_OutputVector , "setVector received bogus stuff" );
  }

  void DisplacementMap::applyFinish ( const AMP::LinearAlgebra::Vector::shared_ptr &  ,
