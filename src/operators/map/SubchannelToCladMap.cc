    ...
    : AsyncMapOperator( p )
{
    AMP::shared_ptr<SubchannelToCladMapParameters> params =
        AMP::dynamic_pointer_cast<SubchannelToCladMapParameters>( p );
    AMP_ASSERT( params );
    ...
    // Get the iterators
    if ( d_mesh1.get() != NULL ) d_iterator1 = getSubchannelIterator( d_mesh1 );
    if ( d_mesh2.get() != NULL ) {
    ...
    for ( size_t i = 0; i < N_subchannels; i++ ) {
        ...
        for ( size_t j = 0; j < tmp.size(); j++ ) {
            if ( tmp[j] == 1 ) d_subchannelRecv[i].push_back( j );
        }
    }
    ...
    if ( d_mesh1.get() != NULL ) {
        for ( size_t i = 0; i < N_subchannels; i++ ) {
            if ( d_ownSubChannel[i] ) d_sendBuffer[i] = std::vector<double>( d_z.size() );
        }
    }
}
...
{
    if ( t == "SubchannelToCladMap" ) return true;
    return false;
}
                                      ...
                                      AMP::LinearAlgebra::Vector::shared_ptr )
{
    ...
    AMP::LinearAlgebra::Vector::const_shared_ptr commSubsetVec =
        u->constSelect( commSelector, var->getName() );
    ...
    for ( size_t i = 0; i < N_subchannels; i++ ) {
        ...
        for ( size_t j = 0; j < d_z.size(); j++ ) d_sendBuffer[i][j] = 0.0;
    }
