...
namespace AMP {
  namespace Operator {
    ...
    VonMisesElastoPlasticModel :: VonMisesElastoPlasticModel (const boost::shared_ptr<MechanicsMaterialModelParameters> & params)
...
//        for(size_t j=0; j<6; j++) d_constitutiveMatrix[i][j] = 0.;
    ...
    void VonMisesElastoPlasticModel :: preNonlinearInit(bool resetReusesRadialReturn, bool jacobianReusesRadialReturn)
    {
    ...
    void VonMisesElastoPlasticModel :: nonlinearResetGaussPointOperation(const std::vector<std::vector<double> >& strain)
    {
    ...
    void VonMisesElastoPlasticModel :: nonlinearResetGaussPointOperation_UL(const std::vector<std::vector<double> >& strain, double R_n[3][3], double R_np1[3][3])
    {
      double stra_np1[6];
    ...
    void VonMisesElastoPlasticModel :: nonlinearJacobianGaussPointOperation(const std::vector<std::vector<double> >& strain)
    {
    ...
    void VonMisesElastoPlasticModel :: nonlinearJacobianGaussPointOperation_UL(const std::vector<std::vector<double> >& strain, double R_n[3][3], double R_np1[3][3])
    {
      double* stress = &(d_tmp2Stress[6*d_gaussPtCnt]);
    ...
    void VonMisesElastoPlasticModel :: getInternalStress(const std::vector<std::vector<double> >& strain, double*& stress)
    {
      double stra_np1[6], Identity[3][3];
    ...
    void VonMisesElastoPlasticModel :: getInternalStress_UL(const std::vector<std::vector<double> >& strain, double*& stress, double R_n[3][3], double R_np1[3][3], double detF)
    {
      double stra_np1[6];
    ...
    void VonMisesElastoPlasticModel :: getConstitutiveMatrixUpdatedLagrangian(double constitutiveMatrix[6][6], double R_np1[3][3])
    {
      constructConstitutiveMatrix();
    ...
    void VonMisesElastoPlasticModel :: constructConstitutiveMatrix()
    {
     ...
     // double eph_bar_plas_np1;
      ...
      AMP_INSIST( ((3.0 * G) + Ep) > tol, "Divide by zero in VonMisesElastoPlasticModel. Line 302");
    ...
    void VonMisesElastoPlasticModel :: radialReturn(const double* stra_np1, double* stre_np1, 
      ...
      if(d_useUpdatedLagrangian == true) {
        if(d_useJaumannRate == false) {
          if(d_iDebugPrintInfoLevel > 11) {
            for(int i = 0; i < 6; i++) {
              AMP::pout<<"d_EquilibriumStress["<<i<<"]="<<d_EquilibriumStress[(6 * d_gaussPtCnt) + i]<<std::endl;
            }
          }
