...
IsotropicElasticModel :: IsotropicElasticModel (const AMP::shared_ptr<MechanicsMaterialModelParameters> & params) :
...
void IsotropicElasticModel :: calculateStress(const std::vector<std::vector<double> >& strain, double*& stress, double R_n[3][3], double R_np1[3][3])
{
    ...
    */
    ...
    if(d_useJaumannRate == false) {
      if(d_iDebugPrintInfoLevel > 11) {
        for(int i = 0; i < 6; i++) {
          std::cout<<"d_EquilibriumStress["<<i<<"]="<<d_EquilibriumStress[(6 * d_gaussPtCnt) + i]<<std::endl;
        }
      }
