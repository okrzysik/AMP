...
namespace Operator {
  ...
  void MechanicsNonlinearElement :: computeStressAndStrain(const std::vector<std::vector<double> > & elementInputVectors, 
...
*/
      ...
      for(int i = 0; i < 6; i++) {
        for(unsigned int j = 0; j < num_nodes; j++) {
          el_np1[i] += (Bl_np1[i][(3 * j) + 0] * elementInputVectors[Mechanics::DISPLACEMENT][(3 * j) + 0]);
          el_np1[i] += (Bl_np1[i][(3 * j) + 1] * elementInputVectors[Mechanics::DISPLACEMENT][(3 * j) + 1]);
          el_np1[i] += (Bl_np1[i][(3 * j) + 2] * elementInputVectors[Mechanics::DISPLACEMENT][(3 * j) + 2]);
        }
      }
  ...
  void MechanicsNonlinearElement :: printStressAndStrain(FILE* fp, const 
...
*/ // This part of the code is used to assign a temperature distribution by hand.        
/*        if(radius <= 0.0025527) {
...
*/
  ...
  void MechanicsNonlinearElement :: apply_Reduced() 
  {
    ...
    for(unsigned int qp = 0; qp < d_qrule->n_points(); qp++) {
      ...
      for(unsigned int i = 0; i < num_nodes; i++) {
        double one3 = 1.0 / 3.0;
        Bl_np1[0][(3 * i) + 0] = dphi[i][qp](0) + (one3 * (avgTraceTerm[(3 * i) + 0] - dphi[i][qp](0)));
        ...
        Bl_np1[1][(3 * i) + 1] = dphi[i][qp](1) + (one3 * (avgTraceTerm[(3 * i) + 1] - dphi[i][qp](1)));
        ...
        Bl_np1[2][(3 * i) + 2] = dphi[i][qp](2) + (one3 * (avgTraceTerm[(3 * i) + 2] - dphi[i][qp](2)));
        Bl_np1[3][(3 * i) + 1] = dphi[i][qp](2);
        Bl_np1[3][(3 * i) + 2] = dphi[i][qp](1);
        Bl_np1[4][(3 * i) + 0] = dphi[i][qp](2);
        Bl_np1[4][(3 * i) + 2] = dphi[i][qp](0);
...
/*      double dudx = 0;
      ...
      //Strain
      fieldsAtGaussPt[Mechanics::DISPLACEMENT].push_back(dudx + ((avgDivDisp - divDisp)/3.0));
      fieldsAtGaussPt[Mechanics::DISPLACEMENT].push_back(dvdy + ((avgDivDisp - divDisp)/3.0));
      fieldsAtGaussPt[Mechanics::DISPLACEMENT].push_back(dwdz + ((avgDivDisp - divDisp)/3.0));
      fieldsAtGaussPt[Mechanics::DISPLACEMENT].push_back(0.5*(dvdz + dwdy));
      fieldsAtGaussPt[Mechanics::DISPLACEMENT].push_back(0.5*(dudz + dwdx));
      fieldsAtGaussPt[Mechanics::DISPLACEMENT].push_back(0.5*(dudy + dvdx));
*/
...
/*          // Strain-Displacement Relation (Voigt Notation) 
          ...
          for(int c = 0; c < 3; c++) {
            tmpStrain[c] = tmpStrain[c] + ((avgTraceTerm[(3*j) + d] - dphi[j][qp](d))/3.0);
          }//end for c
