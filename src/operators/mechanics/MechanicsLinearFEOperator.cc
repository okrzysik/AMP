      ...
      if(d_useUpdatedLagrangian) {
        ...
        for(unsigned int r = 0; r < numNodesInCurrElem; r++) {
          for(unsigned int d = 0; d < 3; d++) {
            if(d_dispVec != NULL) {
              elementInputVectors[Mechanics::DISPLACEMENT][(3*r) + d] = d_dispVec->getValueByGlobalID( d_dofIndices[r][d] );
            } else {
        ...
        for(unsigned int dr = 0; dr < 3; dr++) {
          for(unsigned int c = 0; c < numNodesInCurrElem; c++) {
            for(unsigned int dc = 0; dc < 3; dc++) {
              d_matrix->addValueByGlobalID( d_dofIndices[r][dr], d_dofIndices[c][dc], 
