...
namespace AMP {
  namespace Operator {
    ...
    ThermalStrainMaterialModel :: ThermalStrainMaterialModel (const boost::shared_ptr<MechanicsMaterialModelParameters> & params)
    ...
    void ThermalStrainMaterialModel :: nonlinearResetGaussPointOperation(const std::vector<std::vector<double> >& strain)
    {
      double strain_np1[6];
    ...
    void ThermalStrainMaterialModel :: nonlinearResetGaussPointOperation_UL(const std::vector<std::vector<double> >& strain, double R_n[3][3], double R_np1[3][3])
    {
      double strain_np1[6];
    ...
    void ThermalStrainMaterialModel :: nonlinearJacobianGaussPointOperation(const std::vector<std::vector<double> >& strain)
    {
    ...
    void ThermalStrainMaterialModel :: getInternalStress(const std::vector<std::vector<double> >& strain, double*& stress)
    { 
    ...
    void ThermalStrainMaterialModel :: getInternalStress_UL(const std::vector<std::vector<double> >& strain, double*& stress, double R_n[3][3], double R_np1[3][3], double detF)
    { 
      AMP_INSIST( (d_Is_Source == false), "Inconsistent options!");
    ...
    void ThermalStrainMaterialModel :: constructConstitutiveMatrix(const double passed_E, const double passed_Nu)
    {
      double E = passed_E;
      double Nu = passed_Nu;
    ...
    void ThermalStrainMaterialModel :: constructConstitutiveMatrixUpdatedLagrangian(const double passed_E, const double passed_Nu)
    {
      double E = passed_E;
      double Nu = passed_Nu;
    ...
    void ThermalStrainMaterialModel :: Thermal_Strain_Gauss_Point(const double* stra_np1, const double Temp, double* stre_np1,
        const std::vector<std::vector<double> >& strain, double R_n[3][3], double R_np1[3][3])
    {
      ...
      if(d_useUpdatedLagrangian == true) {
        ...
        if(d_useJaumannRate == true) {
          ...
          stress_n[3] = d_EquilibriumStress[(6 * d_gaussPtCnt) + 3] + (0.5 * (S[1][2] + S[2][1]));
          stress_n[4] = d_EquilibriumStress[(6 * d_gaussPtCnt) + 4] + (0.5 * (S[0][2] + S[2][0]));
          stress_n[5] = d_EquilibriumStress[(6 * d_gaussPtCnt) + 5] + (0.5 * (S[0][1] + S[1][0]));
        }
      }
      ...
      for(int i = 0; i < 3; i++) {
        delta_thermal_strain[i] += thermal_expansion_coefficient * (Temperature_np1 - Temperature_n);
      }
      ...
      if(d_useUpdatedLagrangian == false) {
        ...
        for(int i = 0; i < 6; i++) {
          for(int j = 0; j < 6; j++) {
            delta_thermal_stress[i] += d_constitutiveMatrix_UL[i][j] * (stra_np1[j] - delta_thermal_strain[j]);
          }
        }
        for(int i = 0; i < 6; i++)  {
