...
namespace AMP {
  namespace Operator {
    ...
    PericElastoViscoPlasticModel :: PericElastoViscoPlasticModel (const boost::shared_ptr<MechanicsMaterialModelParameters> & params)
    ...
    void PericElastoViscoPlasticModel :: preNonlinearInit(bool resetReusesRadialReturn, bool jacobianReusesRadialReturn)
    {
    ...
    void PericElastoViscoPlasticModel :: globalReset()
    {
      ...
      if(d_iDebugPrintInfoLevel > 13) {
        for(int i = 0; i < 8; i++) {
          AMP::pout<<"d_EquilibriumYieldStress["<<i<<"]="<<d_EquilibriumYieldStress[i]<<std::endl;
          AMP::pout<<"d_EquilibriumEffectivePlasticStrain["<<i<<"]="<<d_EquilibriumEffectivePlasticStrain[i]<<std::endl;
        }
      }
    }

    void PericElastoViscoPlasticModel :: nonlinearResetGaussPointOperation(const std::vector<std::vector<double> >& strain)
    {
      ...
      radialReturn( &stra_np1[0], stress, &(d_tmp1YieldStress[d_gaussPtCnt]), 
          &(d_tmp1EffectivePlasticStrain[d_gaussPtCnt]), strain, Identity, Identity);
    }

    void PericElastoViscoPlasticModel :: nonlinearResetGaussPointOperation(const std::vector<std::vector<double> >& strain, double R_n[3][3], double R_np1[3][3])
    {
      double stra_np1[6];
      ...
      radialReturn( &stra_np1[0], stress, &(d_tmp1YieldStress[d_gaussPtCnt]), 
          &(d_tmp1EffectivePlasticStrain[d_gaussPtCnt]), strain, R_n, R_np1);
    }

    void PericElastoViscoPlasticModel :: nonlinearJacobianGaussPointOperation(const std::vector<std::vector<double> >& strain)
    {
      ...
      radialReturn( &stra_np1[0], stress, &(d_tmp2YieldStress[d_gaussPtCnt]), 
          &(d_tmp2EffectivePlasticStrain[d_gaussPtCnt]), strain, Identity, Identity);
    }

    void PericElastoViscoPlasticModel :: nonlinearJacobianGaussPointOperation(const std::vector<std::vector<double> >& strain, double R_n[3][3], double R_np1[3][3])
    {
      double* stress = &(d_tmp2Stress[6*d_gaussPtCnt]);
      ...
      radialReturn( &stra_np1[0], stress, &(d_tmp2YieldStress[d_gaussPtCnt]), 
          &(d_tmp2EffectivePlasticStrain[d_gaussPtCnt]), strain, R_n, R_np1);
    }
    ...
    void PericElastoViscoPlasticModel :: getInternalStress(const std::vector<std::vector<double> >& strain, double*& stress)
    {
      double stra_np1[6], Identity[3][3];
    ...
    void PericElastoViscoPlasticModel :: getInternalStress(const std::vector<std::vector<double> >& strain, double*& stress, double R_n[3][3], double R_np1[3][3], double detF)
    {
      double stra_np1[6];
    ...
    void PericElastoViscoPlasticModel :: getConstitutiveMatrixUpdatedLagrangian(double constitutiveMatrix[6][6], double R_np1[3][3])
    {
      constructConstitutiveMatrix();
      ...
      if(d_useJaumannRate == true) {
        for(int i = 0; i < 6; i++) {
          for(int j = 0; j < 6; j++) {
            d_constitutiveMatrix[i][j] /= d_detULF[d_gaussPtCnt];
          }
        }
    ...
    void PericElastoViscoPlasticModel :: constructConstitutiveMatrix()
    {
      ...
      q_np1 = sqrt((3.0 / 2.0) * ((sig_dev[0] * sig_dev[0]) +
            (sig_dev[1] * sig_dev[1]) +
            (sig_dev[2] * sig_dev[2]) +
      ...
      AMP_INSIST(q_trial > tol, "Divide by zero in PericElastoViscoPlasticModel by parameter q_trial.");
      ...
      if(d_useContinuumTangent == false) {
        ...
        AMP_INSIST((d_useContinuumTangent == false), "Continuum tangent for Peric Viscoplasticity has not been implemented yet.");
        for(int i = 0; i < 3; i++) {
          for(int j = 0; j < 3; j++) {
    ...
    double PericElastoViscoPlasticModel :: calculate_E1(const double lambda, const double G, 
        const double q_np1_trial, const double yield_stress_np1)
    {
        ...
        const double G, const double q_np1_trial, const double yield_stress_np1)
    {
      ...
      term1 = q_np1_trial - (3.0 * G * lambda);
      term2 = d_Delta_Time / ((lambda * d_Viscosity) + d_Delta_Time);
      term3 = (d_Epsilon * d_Delta_Time * d_Viscosity) / (((lambda * d_Viscosity) + d_Delta_Time) * ((lambda * d_Viscosity) + d_Delta_Time));
    ...
    void PericElastoViscoPlasticModel :: radialReturn(const double* stra_np1, double* stre_np1, 
        ...
        double R_n[3][3], double R_np1[3][3])
    {
      ...
      q_trial = sqrt(three2 * ((sig_trial_dev[0] * sig_trial_dev[0]) +
      ...
      if(d_iDebugPrintInfoLevel > 10) {
        AMP::pout << "phi = " << phi << "q_trial = " << q_trial << "sigy_np1 = " << sigy_np1 << "sigy_n = " << sigy_n << std::endl;
      }
      ...
      *el_or_pl = 1;
      ...
      AMP_INSIST(fabs(dE1_dlam) > tol, "The term dE1_dlam in PericElastoViscoPlasticModel is less than zero.");
      d_lam = -E1 / dE1_dlam;
      lam = lam + d_lam;
      for(int i = 0; i < 100; i++) {
    ...
    void PericElastoViscoPlasticModel :: postNonlinearAssembly()
    {
      if(Total_Gauss_Point == 0) {
        ...
        if( d_iDebugPrintInfoLevel > 1 ) {
          std::cout << "Fraction = " << Plastic_Fraction << "% Plastic = " 
            << Plastic_Gauss_Point << " Total = " << Total_Gauss_Point 
            << " Gauss Points." <<  "  " << d_iDebugPrintInfoLevel << std::endl;
        }
      }
    }
