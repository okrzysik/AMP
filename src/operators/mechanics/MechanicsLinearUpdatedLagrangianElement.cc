...
namespace Operator {
  ...
  void MechanicsLinearUpdatedLagrangianElement :: computeStressAndStrain(const std::vector<double> & dispVec, 
  ...
  void MechanicsLinearUpdatedLagrangianElement :: printStressAndStrain(FILE* fp, const std::vector<double> & dispVec) {
    const std::vector<std::vector<RealGradient> > & dphi = (*d_dphi);
    ...
    for(unsigned int qp = 0; qp < d_qrule->n_points(); qp++) {
      ...
      fprintf(fp, "%.12f %.12f %.12f %.12f %.12f %.12f \n", uStress[0], uStress[1], uStress[2], uStress[3], uStress[4], uStress[5]);
      fprintf(fp, "%.12f %.12f %.12f %.12f %.12f %.12f \n\n", uStrain[0], uStrain[1], uStrain[2], uStrain[3], uStrain[4], uStrain[5]);
...
*/      
      ...
      double currentStress[6], materialMatrix[6][6], F_np1[3][3], R_np1[3][3], U_np1[3][3];
      ...
      if(d_useJaumannRate == true) {
        ...
        materialMatrix[3][1] = materialMatrix[3][2] = materialMatrix[1][3] = materialMatrix[2][3] -= currentStress[3];
        materialMatrix[4][0] = materialMatrix[4][2] = materialMatrix[0][4] = materialMatrix[2][4] -= currentStress[4];
        materialMatrix[5][0] = materialMatrix[5][1] = materialMatrix[1][5] = materialMatrix[0][5] -= currentStress[5];
        ...
        for(int d1 = 0; d1 < 3; d1++) {
          for(unsigned int k = 0; k < num_nodes; k++) {
            for (int d2 = 0; d2 < 3; d2++) {
              ...
              elementStiffnessMatrix[(3 * k) + d2][(3 * j) + d1] += (detJ[0] * materialStiffness[(3 * k) + d2][(3 * j) + d1]);
    ...
    for(int i = 0; i < 24; i++) {
      for(int j = 0; j < 24; j++) {
        if(elementStiffnessMatrix[i][j] > 1.0) {
          std::cout<<"Initia case 2 elementStiffnessMatrix = "<<elementStiffnessMatrix[i][j]<<std::endl;
        }
...
/*      for(int i = 0; i < 24; i++) {
        for(int j = 0; j < 24; j++) {
          if(elementStiffnessMatrix[i][j] > 100.0) {
            std::cout<<"Initia case prev-3 elementStiffnessMatrix = "<<elementStiffnessMatrix[i][j]<<" qp = "<<qp<<std::endl;
          }
        }
      }
*/
...
/*      for(int i = 0; i < 24; i++) {
        for(int j = 0; j < 24; j++) {
          if(elementStiffnessMatrix[i][j] > 100.0) {
            std::cout<<"Initia case 3 elementStiffnessMatrix = "<<elementStiffnessMatrix[i][j]<<std::endl;
          }
        }
      }
*/
...
/*      for(int i = 0; i < 24; i++) {
        for(int j = 0; j < 24; j++) {
          if(elementStiffnessMatrix[i][j] > 100.0) {
            std::cout<<"Initia case 4 elementStiffnessMatrix = "<<elementStiffnessMatrix[i][j]<<std::endl;
          }
        }
      }
*/
...
/*      for(int i = 0; i < num_nodes; i++) {
        std::cout<<"Linear-->dN_dxn["<<i<<"]="<<dN_dxn[i]<<" dN_dyn["<<i<<"]="<<dN_dyn[i]<<" dN_dzn["<<i<<"]="<<dN_dzn[i]<<std::endl;
      }
*/
      for(unsigned int i = 0; i < num_nodes; i++) {
        Bl_np1[0][(3 * i) + 0] = dNdx[i];
...
/*      for(int i = 0; i < 24; i++) {
        for(int j = 0; j < 24; j++) {
          if(elementStiffnessMatrix[i][j] > 100.0) {
            std::cout<<"Initia case 5 elementStiffnessMatrix = "<<elementStiffnessMatrix[i][j]<<std::endl;
          }
        }
      }
*/
      double constitutiveMatrix[6][6];
      double currentStress[6], materialMatrix[6][6], R_np1[3][3], F_np1[3][3], U_np1[3][3];
...
/*      for(int i = 0; i < 24; i++) {
        for(int j = 0; j < 24; j++) {
          if(elementStiffnessMatrix[i][j] > 100.0) {
            std::cout<<"Initia case 6 elementStiffnessMatrix = "<<elementStiffnessMatrix[i][j]<<std::endl;
          }
        }
      }
*/
        ...
        for(int d1 = 0; d1 < 3; d1++) {
          ...
          for(unsigned int k = 0; k < num_nodes; k++) {
            for (int d2 = 0; d2 < 3; d2++) {
              ...
              //elementStiffnessMatrix[(3 * k) + d2][(3 * j) + d1] += (detJ_n[0] * materialStiffness[(3 * k) + d2][(3 * j) + d1]);
              elementStiffnessMatrix[(3 * k) + d2][(3 * j) + d1] += (detJ[0] * materialStiffness[(3 * k) + d2][(3 * j) + d1]);
              //AMP::pout<<"elementStiffnessMatrix["<<(3 * k) + d2<<"]["<<(3 * j) + d1<<"]="<<elementStiffnessMatrix[(3 * k) + d2][(3 * j) + d1]<<std::endl;
      ...
      for(unsigned int i = 0; i < (3 * num_nodes); i++) {
          for(unsigned int j = 0; j < (3 * num_nodes); j++) {
            //elementStiffnessMatrix[i][j] += (detJ_n[0] * tempStiffness[i][j]);
            elementStiffnessMatrix[i][j] += (detJ[0] * tempStiffness[i][j]);
            //AMP::pout<<"elementStiffnessMatrix["<<i<<"]["<<j<<"] = "<<elementStiffnessMatrix[i][j]<<std::endl;
        }
      }
