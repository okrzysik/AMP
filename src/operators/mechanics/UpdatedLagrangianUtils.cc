...
namespace Operator {
  ...
  void matDiagMatMultiply(double P1[3][3], double D[3], 
      double P2[3][3], double A[3][3]) {
    double Tmp[3][3];
          ...
          if(softEquals(A[1][1] , 0)) {
            if(softEquals(A[1][2] , 0)) {
              ...
              if(softEquals(A[2][1] , 0)) {
                ...
                std::vector<double> tmp(3);
                tmp[0] = 1;
                tmp[1] = ((A[2][2]*A[1][0]) - (A[2][0]*A[1][2]))/(A[1][2]*A[2][1]);
                tmp[2] = -A[1][0]/A[1][2];
                sols.push_back(tmp);
              }//A21
            }//A12
            ...
            if( softEquals( (A[1][1]*A[2][2]) , (A[2][1]*A[1][2]) ) ) {
              ...
              std::vector<double> tmp(3);
              tmp[0] = 1;
              tmp[2] = ((A[2][1]*A[1][0]) - (A[2][0]*A[1][1]))/((A[2][2]*A[1][1]) - (A[2][1]*A[1][2]));
              tmp[1] = -(A[1][0] + (A[1][2]*tmp[2]))/A[1][1];
              sols.push_back(tmp);
            }
          }//A11
  ...
  void firstCubicRoot(double a, double b, double c, double d, double & r1) {
    ...
    if(softEquals(q , 0)) {
      ...
      t = 2.0*sqrt(-p/3.0)*cos(1.0/3.0*acos((3.0*q/(2.0*p))*sqrt(-3.0/p)));
    }
       ...
       for(j=0; j<3; j++)
       {
           for(k=0; k<3; k++) 
           {
               ...
               Tijab[j3][k3] = (Qia[m1j][m1k] * Qjb[m2j][m2k] +  Qia[m2j][m1k] * Qjb[m1j][m2k] + 
                   Qia[m1j][m2k] * Qjb[m2j][m1k] + Qia[m2j][m2k]*Qjb[m1j][m1k]) * 0.50;
           }
       }
  }
  ...
  void polarDecompositionFeqRU_Simo(double F[3][3], double R[3][3], double U[3][3])  // Decomposes the deformation gradient F into rotation R and stretch U (F = RU) 
  {
    ...
    if(fabs(b) < tol) {
      ...
      AMP_INSIST((c >= 0.0), "Error in the polar decomposition (Simo), value of c is less than zero.");
      for(int i = 0; i < 3; i++)
        x[i] = -pow(c, (1.0 / 3.0));
    } else {
    ...
    for(int i = 0; i < 3; i++) {
      ...
      AMP_INSIST((term2 >= 0.0), "Error in the polar decomposition (Simo), while calculating lambda, lambda^2 is negetive.");
      lambda[i] = sqrt(term2);
    }
    ...
    D = (i1 * i2) - i3;
    AMP_INSIST((D > tol), "Error in the polar decomposition (Simo), D should be greater than zero.");

    for(int i = 0; i < 3; i++) {
      for(int j = 0; j < 3; j++) {
        U[i][j] = (1.0 / D) * (-C2[i][j] + (((i1 * i1) - i2) * C[i][j]) + (i1 * i3 * I[i][j]));
        AMP_INSIST((i3 > tol), "Error in the polar decomposition (Simo), i3 should be greater than zero.");
  ...
  void computeJInverse(double G[3][3], double dNdxi[8], double dNdeta[8], double dNdzeta[8], double x[8], double y[8], double z[8], double detJ[1]) {
    ...
    AMP_INSIST((detJ[0] > 0.0), "\n#########\nVolume of the element is negetive.\nExiting the code.\n##########\n");
