...
namespace Operator {
  ...
  ElasticDamageThermalStrainModel :: ElasticDamageThermalStrainModel (const boost::shared_ptr<MechanicsMaterialModelParameters> & params)
    : MechanicsMaterialModel(params)
  {
    ...
    d_CriticalDamageThreshold = (params->d_db)->getDoubleWithDefault("Critical_Damage_Threshold",0.4);
  ...
  void ElasticDamageThermalStrainModel :: preNonlinearInit(bool resetReusesRadialReturn, bool jacobianReusesRadialReturn)
  {
    ...
    AMP_INSIST( ((d_resetReusesRadialReturn == true) && (d_jacobianReusesRadialReturn == true)), "The current code works only with true values for resetReusesRadialReturn and jacobianReusesRadialReturn");
  ...
  void ElasticDamageThermalStrainModel :: globalReset()
  {
    ...
    if(d_iDebugPrintInfoLevel > 11) {
      AMP::pout<<"d_tmp1Tau[799]="<<d_tmp1Tau[799]<<"  d_tmp1Damage[799]="<<d_tmp1Damage[799]<<std::endl;
      AMP::pout<<"d_tmp1Tau[599]="<<d_tmp1Tau[599]<<"  d_tmp1Damage[599]="<<d_tmp1Damage[599]<<std::endl;
      AMP::pout<<"d_tmp1Tau[853]="<<d_tmp1Tau[853]<<"  d_tmp1Damage[853]="<<d_tmp1Damage[853]<<std::endl;
      AMP::pout<<"d_tmp1Tau[478]="<<d_tmp1Tau[478]<<"  d_tmp1Damage[478]="<<d_tmp1Damage[478]<<std::endl;
      AMP::pout<<"d_InitialDamageVec[799]="<<d_InitialDamageVec[799]<<"  d_CriticalDamageVec[799]="<<d_CriticalDamageVec[799]<<std::endl;
      AMP::pout<<"d_InitialDamageVec[599]="<<d_InitialDamageVec[599]<<"  d_CriticalDamageVec[599]="<<d_CriticalDamageVec[599]<<std::endl;
      AMP::pout<<"d_InitialDamageVec[853]="<<d_InitialDamageVec[853]<<"  d_CriticalDamageVec[853]="<<d_CriticalDamageVec[853]<<std::endl;
      AMP::pout<<"d_InitialDamageVec[478]="<<d_InitialDamageVec[478]<<"  d_CriticalDamageVec[478]="<<d_CriticalDamageVec[478]<<std::endl;
    }
  }

  void ElasticDamageThermalStrainModel :: nonlinearResetGaussPointOperation(const std::vector<std::vector<double> >& strain)
  {
    double strain_np1[6];
  ...
  void ElasticDamageThermalStrainModel :: nonlinearJacobianGaussPointOperation(const std::vector<std::vector<double> >& strain)
  {
    if(d_useMaterialsLibrary == true) {
      computeEvalv(strain);
  ...
  void ElasticDamageThermalStrainModel :: getInternalStress(const std::vector<std::vector<double> >& strain, double*& stress)
  { 
...
*/
  ...
  void ElasticDamageThermalStrainModel :: constructConstitutiveMatrix(const double passed_E, const double passed_Nu)
  {
    double E = passed_E;
    double Nu = passed_Nu;
  ...
  void ElasticDamageThermalStrainModel :: Thermal_Strain_Gauss_Point(const double* stra_np1, const double Temp, double* stre_np1,
      const std::vector<std::vector<double> >& strain)
  {
    ...
    if(d_checkCladOrPellet == false) {
      delta_thermal_strain[0] += thermal_expansion_coefficient * (Temperature_np1 - Temperature_n);
      delta_thermal_strain[1] += thermal_expansion_coefficient * (Temperature_np1 - Temperature_n);
      delta_thermal_strain[2] += thermal_expansion_coefficient * (Temperature_np1 - Temperature_n);
    } else {
      if(d_useMaterialsLibrary == true) {
        delta_thermal_strain[0] += (d_tmp1ThermalStrain_Radial[d_gaussPtCnt] - d_EquilibriumThermalStrain_Radial[d_gaussPtCnt]);
        delta_thermal_strain[1] += (d_tmp1ThermalStrain_Radial[d_gaussPtCnt] - d_EquilibriumThermalStrain_Radial[d_gaussPtCnt]);
        delta_thermal_strain[2] += (d_tmp1ThermalStrain_Axial[d_gaussPtCnt] - d_EquilibriumThermalStrain_Axial[d_gaussPtCnt]);
      }
    }
    ...
    if(d_tmp1Tau[d_gaussPtCnt] <= d_EquilibriumDamageThreshold[d_gaussPtCnt]) {
      ...
      d_tmp1Damage[d_gaussPtCnt] = d_EquilibriumDamage[d_gaussPtCnt] + (d_tmp1Tau[d_gaussPtCnt] - d_EquilibriumTau[d_gaussPtCnt]);
      if(d_tmp1Damage[d_gaussPtCnt] > d_CriticalDamageVec[d_gaussPtCnt]) d_tmp1Damage[d_gaussPtCnt] = d_CriticalDamageVec[d_gaussPtCnt];
      double dam = d_tmp1Damage[d_gaussPtCnt];
      for(int i = 0; i < 6; i++) {
        for(int j = 0; j < 6; j++) {
          d_constitutiveMatrix[i][j] = (1.0 - dam) * d_initialConstitutiveMatrix[i][j];
    ...
    if(d_Is_Source) {
      ...
      for(int i = 0; i < 6; i++) {
        for(int j = 0; j < 6; j++) {
          delta_thermal_stress[i] += d_constitutiveMatrix[i][j] * (stra_np1[j] - stra_n[j] - delta_thermal_strain[j]);
          delta_thermal_stress_1[i] += d_initialConstitutiveMatrix[i][j] * (stra_np1[j] - stra_n[j] - delta_thermal_strain[j]);
        }
      }
      ...
      if(d_iDebugPrintInfoLevel > 15) {
        ...
        std::cout << "vec_norm = " << vec_norm << "    vec_norm_1 = " << vec_norm_1 << std::endl;
      }
