...
namespace Operator {
  ...
  void polarDecompositionFeqRU_Simo(double F[3][3], double R[3][3], double U[3][3]); // Decomposes the deformation gradient F into rotation R and stretch U (F = RU) 

  // This function calculates the derivatives of the shape functions with respect to the global x, y and z directions. 
  // dNdx, dNdy and dNdz are the output arguments which contains the derivatives of the shape functions.
  ...
  // detJ contains the determinant of the jacobian matrix. It is used in the numerical integration. This is an output.
  ...
  // This function calculates values of all the 8 shape functions at a particular gauss/quadrature point.
  // xi, eta and zeta are the coordinates of the current gauss point. These are given as input.
  // N holds the 8 shape functions. It is the output.
  void computeShapeFunctions(double N[8], double xi, double eta, double zeta);

  // This function calculates the derivatives of the shape function with respect to the isoparametric coordinates xi, eta and zeta. 
  ...
  // dNdxi, dNdeta and dNdzeta are the local derivatives of the shape functions. These are given as inputs.
  // x, y and z contains the nodal coordinates for the current element. These are inputs.
  // detJ is an output. It contains the determinant of the J matrix. It also signifies the volume of the current element. 
  ...
  // This function calculates the symmetric part of the deformation gradient also known as the rate of deformation tensor (d_np1o2).
  // dN_dxnp1o2, dN_dynp1o2 and dN_dznp1o2 are the derivatives of the shape functions with respect to the np1o2 configuration.
  // delta_u, delta_v and delta_w are the incremental displacements between n-th and (n+1)-th configuration.
