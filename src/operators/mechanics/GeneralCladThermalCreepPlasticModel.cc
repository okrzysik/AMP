...
namespace Operator {
  ...
  GeneralCladThermalCreepPlasticModel :: GeneralCladThermalCreepPlasticModel (const boost::shared_ptr<MechanicsMaterialModelParameters> & params)
    : MechanicsMaterialModel(params)
  {
    ...
    AMP_INSIST( (d_useMaterialsLibrary == true), "For GeneralCladThermalCreepPlasticModel, the materials library must be used.");
    ...
    default_alpha = (params->d_db)->getDoubleWithDefault("THERMAL_EXPANSION_COEFFICIENT", 2.0e-6);
  ...
  void GeneralCladThermalCreepPlasticModel :: nonlinearInitGaussPointOperation(double tempAtGaussPoint)
  {
  ...
  void GeneralCladThermalCreepPlasticModel :: nonlinearResetGaussPointOperation(const std::vector<std::vector<double> >& strain)
  {
    ...
    if(d_UseThermalStrain == true) {
      double thermal_strain = d_tmp1ThermalStrain[d_gaussPtCnt] - d_EquilibriumThermalStrain[d_gaussPtCnt];
    ...
    if(d_UseCreepStrain == true) {
      ...
      computeCreepStrain(Temp_np1, stress_n, creep_strain_prev, delta_creep_strain, net_stra_np1);
  ...
  void GeneralCladThermalCreepPlasticModel :: nonlinearJacobianGaussPointOperation(const std::vector<std::vector<double> >& strain)
  {
  ...
  void GeneralCladThermalCreepPlasticModel :: getInternalStress(const std::vector<std::vector<double> >& strain, double*& stress)
  {
  ...
  double GeneralCladThermalCreepPlasticModel :: help_compute_E1(const double Temp_np1, double effective_stress)
  {
      ...
      double effective_stress, double Temp_np1, double G)
  {
    ...
    double term2 = (effective_stress * effective_stress_trial) / ((effective_stress + (3.0 * G * delta_e)) * (effective_stress + (3.0 * G * delta_e)));

    effective_stress = effective_stress * scale_stress;
    double d_delta_e_d_sig_e = A * dt * exp(-Q / (R * Temp_np1)) * n * pow(effective_stress, (n - 1.0)) * scale_strain;
  ...
  void GeneralCladThermalCreepPlasticModel :: computeCreepStrain(const double Temp_np1, const double stress_n[6], 
      const double creep_strain_prev, double delta_creep_strain[6], double net_stra_np1[6])
  {
    ...
    if(d_iDebugPrintInfoLevel > 9) {
      std::cout << "currentTime = " << d_currentTime << " previousTime = " << d_previousTime << std::endl;
    }
    ...
    for(int i = 0; i < 6; i++) {
      deviatoric_stress_trial[i] = deviatoric_stress[i] + (2.0 * G * deviatoric_delta_strain[i]);
    }
    
    effective_stress_trial = sqrt(three2 * ((deviatoric_stress_trial[0] * deviatoric_stress_trial[0]) +
    ...
    for(int jkl = 0; jkl < 1000; jkl++) {
      double delta_e = help_compute_E1(Temp_np1, effective_stress);
      double E1 = effective_stress - (effective_stress_trial / (1.0 + ((3.0 * G * delta_e) / effective_stress)));
      double dE1_dsig_e = compute_dE1_dsig_e(effective_stress_trial, effective_stress, Temp_np1, G);
    ...
    if(newton_converged == 0) {
      ...
      for(int jkl = 0; jkl < 1000; jkl++) {
        ...
        double E1_1 = effective_stress_1 - (effective_stress_trial / (1.0 + ((3.0 * G * delta_e_1) / effective_stress_1)));
  
        double delta_e_2 = help_compute_E1(Temp_np1, effective_stress_2);
        double E1_2 = effective_stress_2 - (effective_stress_trial / (1.0 + ((3.0 * G * delta_e_2) / effective_stress_2)));
        ...
        if(((det_sign(E1) * det_sign(E1_1)) < 0) && ((det_sign(E1) * det_sign(E1_2)) > 0)) {
          effective_stress_2 = effective_stress;
        }

        if(((det_sign(E1) * det_sign(E1_2)) < 0) && ((det_sign(E1) * det_sign(E1_1)) > 0)) {
    ...
    if((bisection_converged == 0) && (newton_converged == 0)) {
      std::cout << "Creep code did not converge for both newton and bisection iterations." << std::endl;
      exit(1);
    }
    ...
    for(int i = 0; i < 6; i++) {
      if(effective_stress > tol) {
        deviatoric_stress[i] = (effective_stress * deviatoric_stress_trial[i]) / (effective_stress + (3.0 * G * delta_e));
      } else {
  ...
  void GeneralCladThermalCreepPlasticModel :: computeEvalv(const std::vector<std::vector<double> >& strain)
  {
  ...
  void GeneralCladThermalCreepPlasticModel :: radialReturn(const double* stra_np1, double* stre_np1, 
