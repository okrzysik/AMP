...
public :
    ...
    //! Constructor.
    explicit MechanicsNonlinearUpdatedLagrangianElement(const AMP::shared_ptr<ElementOperationParameters>& params) :
    ...
    void setElementVectors( const std::vector<std::vector<double> > & elementInputVectors, 
        const std::vector<std::vector<double> > & elementInputVectors_pre, std::vector<double> & elementOutputVector )
    {
    ...
    /**
      This function is used to initialize the material model at all Gauss points within the current element. 
      ...
      Currently, the only field required by the mechanics material models during initialization is the reference temperature.
      @param [in] initTempVector Reference temperature at the nodes of the current element. This can be 
    ...
    /**
      This function is used to update the equilibrium values of stress, strain, temperature, burnup etc that are
      stored in the material model class. This is typically required at the end of each loading step. This function is typically 
      called from within the MechanicsNonlinearFEOperator's reset function, which is evaluated at the end of each loading step.  
      This function also gets called from within the MechanicsNonlinearFEOperator's getJacobianParameters 
      function if the jacobian is evaluated at a state (displacement, temperature etc) different from 
    ...
    /**
      ...
      @param [in] inputVec Input vector (Displacement, Temperature, Burnup etc) at the nodes of the current element.
     */
...
protected :
    ...
    template <MaterialUpdateType type>
    void materialModelNonlinearGaussPointOperation(const std::vector<std::vector<double> >&, double[3][3], double[3][3] );
    ...
    std::vector<std::vector<double> > d_elementInputVectors_pre; /**< Element input vectors at the previous time step (Displacement, temperature, burnup etc). */
...
template <>
inline void MechanicsNonlinearUpdatedLagrangianElement :: materialModelPreNonlinearGaussPointOperation<
MechanicsNonlinearUpdatedLagrangianElement::RESET>()
{
...
template <>
inline void MechanicsNonlinearUpdatedLagrangianElement :: materialModelPostNonlinearGaussPointOperation<
MechanicsNonlinearUpdatedLagrangianElement::RESET>()
{
...
MechanicsNonlinearUpdatedLagrangianElement::RESET>(const std::vector<std::vector<double> >& strain, double R_n[3][3], double R_np1[3][3])
{
...
template <>
inline void MechanicsNonlinearUpdatedLagrangianElement :: materialModelNonlinearGaussPointOperation<
MechanicsNonlinearUpdatedLagrangianElement::JACOBIAN>(const std::vector<std::vector<double> >& strain, double R_n[3][3], double R_np1[3][3])
{
    ...
    const std::vector<std::vector<double> > & elementInputVectors_pre )
{
    ...
    for(unsigned int ijk = 0; ijk < num_nodes; ijk++) {
        ...
        xyz_n[ijk](1) = xyz[ijk](1) + elementInputVectors_pre[Mechanics::DISPLACEMENT][(3*ijk) + 1];
        xyz_n[ijk](2) = xyz[ijk](2) + elementInputVectors_pre[Mechanics::DISPLACEMENT][(3*ijk) + 2];
