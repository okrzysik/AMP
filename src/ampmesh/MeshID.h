...
public:
	...
	MeshID(unsigned int root, unsigned int local_id) {
        // Use the first 32-bits for the rank of the root processor and the second 32-bits for the local id
        data = root;
...
public:
	...
	MeshElementID() {
        ...
        data[1] = 0x000000FFFFFFFFFF;       // set is_local to false, type_id to 0xFF, owner_rank to 0, and local_id to -1
    }
	MeshElementID(bool isLocal, GeomType type_id, unsigned int local_ID, unsigned int owner_rank_id, MeshID mesh_ID) {
        // Copy the meshID
        data[0] = mesh_ID.data;
