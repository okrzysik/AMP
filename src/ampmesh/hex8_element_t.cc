...
bool hex8_element_t::within_bounding_box(double const *p, double tolerance) 
{
    ...
    for (unsigned int j = 0; j < 3; ++j) {
        ...
        if ( (bounding_box[j+0] - tolerance*(bounding_box[j+3]-bounding_box[j+0]) > p[j]) || 
             (bounding_box[j+3] + tolerance*(bounding_box[j+3]-bounding_box[j+0]) < p[j])) 
        { 
            return false; 
        }
    } // end for j
...
void hex8_element_t::build_bounding_polyhedron() 
{
    ...
    for (unsigned int i = 0; i < 6; ++i) {
        ...
        if (tmp_triangles_ptr[0]->above_point(tmp_triangles_ptr[2]->get_centroid(), tolerance)) {
            AMP_ASSERT(tmp_triangles_ptr[0]->above_point(tmp_triangles_ptr[3]->get_centroid(), tolerance));
            AMP_ASSERT(tmp_triangles_ptr[1]->above_point(tmp_triangles_ptr[2]->get_centroid(), tolerance));
            AMP_ASSERT(tmp_triangles_ptr[1]->above_point(tmp_triangles_ptr[3]->get_centroid(), tolerance));
            ...
            AMP_ASSERT(tmp_triangles_ptr[2]->above_point(tmp_triangles_ptr[0]->get_centroid(), tolerance));
            AMP_ASSERT(tmp_triangles_ptr[2]->above_point(tmp_triangles_ptr[1]->get_centroid(), tolerance));
            AMP_ASSERT(tmp_triangles_ptr[3]->above_point(tmp_triangles_ptr[0]->get_centroid(), tolerance));
            AMP_ASSERT(tmp_triangles_ptr[3]->above_point(tmp_triangles_ptr[1]->get_centroid(), tolerance));
