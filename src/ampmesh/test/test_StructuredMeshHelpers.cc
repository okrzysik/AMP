...
void runTest( AMP::UnitTest *ut )
{
    ...
    if ( mesh->getComm().getSize() > 1 ) {
        bool pass = false;
        for (int d=0; d<3; d++) {
            AMP::Mesh::MeshIterator face0 = AMP::Mesh::StructuredMeshHelper::getYZFaceIterator(mesh,0);
            AMP::Mesh::MeshIterator face1 = AMP::Mesh::StructuredMeshHelper::getYZFaceIterator(mesh,1);
