...
MACRO( CONFIGURE_TPLs )
    ...
    IF ( EIGEN3_INSTALL_DIR )
        SET( USE_EXT_EIGEN true )
        SET( EIGEN_DIRECTORY "${EIGEN3_INSTALL_DIR}" )
    ENDIF()
    ...
    ENDIF()
    CHECK_ENABLE_FLAG( USE_EXT_OGRE 0 )
    IF ( OGRE_INSTALL_DIR )
        SET( USE_EXT_OGRE true )
        SET( OGRE_DIRECTORY "${OGRE_INSTALL_DIR}" )
    ENDIF()
    CHECK_ENABLE_FLAG( USE_TIMER 0 )
    IF ( TIMER_INSTALL_DIR )
        SET( USE_TIMER true )
...
MACRO ( CONFIGURE_MPI )
    ...
    IF ( USE_EXT_MPI )
        ...
        IF ( USE_EXT_MPI_FOR_SERIAL_TESTS )
            SET( USE_MPI_FOR_SERIAL_TESTS 1 )
        ENDIF()
...
MACRO ( CONFIGURE_EIGEN_LIBRARIES )
    ...
    IF ( USE_EXT_EIGEN )
        ...
        ENDIF()
        ADD_DEFINITIONS ( "-D USE_EXT_EIGEN" )  
    ENDIF()
ENDMACRO ()
...
# Macro to find and configure the OGRE package
MACRO ( CONFIGURE_OGRE )
    # Determine if we want to use Eigen
    CHECK_ENABLE_FLAG( USE_EXT_OGRE 0 )
    IF ( USE_EXT_OGRE )
        ADD_DEFINITIONS( "-D USE_EXT_OGRE" )  
        IF ( OGRE_DIRECTORY )
            VERIFY_PATH( ${OGRE_DIRECTORY} )
            VERIFY_PATH( ${OGRE_DIRECTORY}/include/OGRE )
            INCLUDE_DIRECTORIES( /usr/include/OIS )
            INCLUDE_DIRECTORIES( ${OGRE_DIRECTORY}/include/OGRE )
            INCLUDE_DIRECTORIES( ${OGRE_DIRECTORY}/include/OGRE/Overlay )
        ELSE()
            MESSAGE( FATAL_ERROR "Default search for OGRE is not yet supported.  Use -D OGRE_DIRECTORY=" )
        ENDIF()
        MESSAGE( "Using ogre" )
    ENDIF()
ENDMACRO ()
...
MACRO ( CONFIGURE_AMP )
    ...
    SET( ${PROJECT_NAME}_ENABLE_AMP_GRAPHICS        OFF )
    IF ( USE_AMP_UTILS )
    ...
    ENDIF()
    IF ( USE_AMP_GRAPHICS )
        SET( ${PROJECT_NAME}_ENABLE_AMP_GRAPHICS ON )
        SET( AMP_GRAPHICS_ENABLE_TESTS ON )
    ENDIF()
    ...
    ENDIF()
    IF ( USE_AMP_GRAPHICS )
        SET ( AMP_DOC_DIRS "${AMP_DOC_DIRS}  \"${AMP_SOURCE_DIR}/src/graphics\"" )
    ENDIF()
    # Set the flags
