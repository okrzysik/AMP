...
MACRO ( CONFIGURE_TRILINOS_LIBRARIES )
    ...
    IF ( USE_TRILINOS )
        ...
        IF ( TRILINOS_DIRECTORY )
            ...
            IF ( (NOT TRILINOS_EPETRA_LIB) OR (NOT TRILINOS_EPETRAEXT_LIB) OR (NOT TRILINOS_AZTECOO_LIB) OR (NOT TRILINOS_ML_LIB) OR 
                 ...
                 (NOT TRILINOS_NOX_LIB) OR (NOT TRILINOS_TEUCHOS_LIB) OR (NOT TRILINOS_MOERTEL_LIB) )
    ...
    ELSE()
        MESSAGE ( WARNING "Trilinos is currently required for AMP to function properly, only a very small subset of AMP will be enabled" )
    ENDIF()
ENDMACRO ()
...
MACRO ( CONFIGURE_LIBMESH )
    ...
    ELSE()
        MESSAGE ( WARNING "Libmesh is currently required for AMP to function properly, only a very small subset of AMP will be enabled" )
    ENDIF()
ENDMACRO ()
...
MACRO ( CONFIGURE_BLAS )
    ...
    IF ( USE_BLAS )
        ...
        MESSAGE ( "   " ${BLAS_LIBS} )
    ELSE()
        MESSAGE ( WARNING "BLAS is currently required for AMP to function properly, only a very small subset of AMP will be enabled" )
    ENDIF()
ENDMACRO ()
...
MACRO ( CONFIGURE_LAPACK )
    ...
    IF ( USE_LAPACK )
        ...
        MESSAGE ( "   " ${LAPACK_LIBS} )
    ELSE()
        MESSAGE ( WARNING "LAPACK is currently required for AMP to function properly, only a very small subset of AMP will be enabled" )
    ENDIF()
ENDMACRO ()
...
MACRO ( CONFIGURE_PETSC_LIBRARIES )
    ...
    IF ( USE_PETSC )
        ...
        MESSAGE ( "   "  ${PETSC_LIBS} )
    ELSE()
        MESSAGE ( WARNING "PETSc is currently required for AMP to function properly, only a very small subset of AMP will be enabled" )
    ENDIF()
ENDMACRO ()
...
MACRO ( CONFIGURE_AMP )
    ...
    ENDIF()
    # Check if we are using descritization
    CHECK_ENABLE_FLAG( USE_AMP_DESCRITIZATION 1 )
    IF ( NOT USE_AMP_DESCRITIZATION )
        MESSAGE ( "Disabling AMP Descritization" )
    ENDIF()
    # Check if we are using vectors
    CHECK_ENABLE_FLAG( USE_AMP_VECTORS 1 )
    IF ( (NOT USE_PETSC) OR (NOT USE_TRILINOS) )
        SET ( USE_AMP_VECTORS 0 )
    ENDIF()
    ...
    ENDIF()
    IF ( USE_AMP_DESCRITIZATION )
        SET ( AMP_LIBS ${AMP_LIBS} "descritization" )
        ADD_DEFINITIONS ( -D USE_AMP_DESCRITIZATION )  
    ENDIF()
