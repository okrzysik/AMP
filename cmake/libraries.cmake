...
MACRO ( CONFIGURE_BOOST )
    ...
    IF ( USE_EXT_BOOST )
        ...
        IF ( BOOST_DIRECTORY )
            ...
            SET( Boost_LIBRARY_DIR_DEBUG=${BOOST_DIRECTORY}/lib )
            SET( Boost_LIBRARY_DIR_RELEASE=${BOOST_DIRECTORY}/lib )
            SET( CMAKE_INSTALL_RPATH ${CMAKE_INSTALL_RPATH} "${BOOST_DIRECTORY}/lib" )
        ENDIF()
        FIND_PACKAGE( Boost COMPONENTS thread date_time system QUIET )
...
MACRO ( CONFIGURE_OGRE )
    ...
    IF ( USE_EXT_OGRE )
        ...
        IF ( OGRE_DIRECTORY )
            ...
            VERIFY_PATH( "${OGRE_DIRECTORY}/lib/OGRE/cmake" )
        	SET( CMAKE_MODULE_PATH "${OGRE_DIRECTORY}/lib/OGRE/cmake" ${CMAKE_MODULE_PATH} )
            SET( OGRE_HOME "${OGRE_DIRECTORY}" )
            SET( ENV_ZLIB_HOME ${ZLIB_DIRECTORY} )
            FIND_PACKAGE( OGRE REQUIRED )
            FIND_PACKAGE( OIS REQUIRED )
            INCLUDE_DIRECTORIES( ${OIS_INCLUDE_DIRS} ${OGRE_INCLUDE_DIRS} ${OGRE_SAMPLES_INCLUDEPATH} ${OGRE_Overlay_INCLUDE_DIRS} )
            KEEP_BUILD_LIBRARIES( OGRE_LIBRARIES )
            KEEP_BUILD_LIBRARIES( OIS_LIBRARIES )
            KEEP_BUILD_LIBRARIES( OGRE_Overlay_LIBRARIES )
            SET( OGRE_LIBS ${OGRE_LIBRARIES} ${OIS_LIBRARIES} ${OGRE_Overlay_LIBRARIES} )
            IF ( NOT USE_EXT_X11 )
                SET( USE_EXT_X11 TRUE )
...
MACRO ( CONFIGURE_AMP )
    ...
    SET( TPL_LIBS ${OGRE_LIBS} ${NEK_LIBS} ${MOAB_LIBS} ${DENDRO_LIBS} ${NETCDF_LIBS}
        ${LIBMESH_LIBS} ${TRILINOS_LIBS} ${PETSC_LIBS} ${HYPRE_LIBS} ${SILO_LIBS}
        ${HDF5_LIBS} ${TIMER_LIBS} ${X11_LIBS} ${SUNDIALS_LIBS} ${ZLIB_LIBS} ${BOOST_LIBS} ${CUDA_LIBRARIES} )
    MESSAGE("TPL_LIBS=${TPL_LIBS}")
